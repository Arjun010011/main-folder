...
class ApproveViewSetTest(TestCase):
    ...
    def test_delete_all_feetypes_pos(self):
        response = client.post(reverse('approve-list'), data=json.dumps(self.valid_payload),
                               content_type='application/json')
        self.assertEqual(response.data['Reason'], 'Fee plan is Approved Successfully!')
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_delete_all_feetypes_neg(self):
        self.fs.is_approved = '1'
        self.fs.save()
        response = client.post(reverse('approve-list'), data=json.dumps(self.invalid_payload),
                               content_type='application/json')
        self.assertEqual(response.data[0], 'Fee plan is already Approved!')
