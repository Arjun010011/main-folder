...
from apps.gallery.models import GalleryTreeItem
...
from apps.gallery.serializers import  (
    GalleryTreeItemGroupPermissionReadSerializer, GalleryTreeItemSerializer, GalleryTreeItemStandardPermissionReadSerializer, GalleryTreeItemStandardSectionReadPermissionSerializer, GalleryTreeItemUserPermissionSerializer, GalleryTreeItemGroupPermissionSerializer, 
    GalleryTreeItemStandardPermissionSerializer, GalleryTreeItemStandardSectionPermissionSerializer, GalleryTreeItemUserReadPermissionSerializer
)
from apps.shared.services import SharedService, UploadTypeService
from apps.gallery.services import default_service
from apps.gallery.services.permissions import (
    add_gallery_standard_section_permission, add_gallery_group_permission, add_gallery_user_permission,
    add_gallery_standard_permission, copy_permission
)

class CreateGalleryFolderViewSet(viewsets.ModelViewSet):
    serializer_class = GalleryTreeItemSerializer
    http_method_names = ['post', 'delete', 'put']

    def get_queryset(self):
        self.queryset = GalleryTreeItem.objects.filter(is_active=True)
...
class CreateGalleryFileViewSet(viewsets.ModelViewSet):
    serializer_class = GalleryTreeItemSerializer
    http_method_names = ['post', 'delete', 'put']
...
class GetGalleryFolderViewSet(viewsets.ModelViewSet):
    serializer_class = GalleryTreeItemSerializer
    queryset = GalleryTreeItem.objects.filter(is_active=True)
...
class GetGalleryFolderImgVideoViewSet(viewsets.ModelViewSet):
    serializer_class = GalleryTreeItemSerializer
    queryset = GalleryTreeItem.objects.filter(is_active=True)
    http_method_names = ['get']
...
class MoveGalleryFolderOrFileViewSet(viewsets.ModelViewSet):
    serializer_class = GalleryTreeItemSerializer
    queryset = GalleryTreeItem.objects.filter()
...
class GetGalleryTreeStructureViewSet(viewsets.ModelViewSet):
    serializer_class = GalleryTreeItemSerializer
    http_method_names = ['get']
    ...
    def list(self, request, *args, **kwargs):
        treeObj = GalleryTreeItem.objects.get(id=1)
        response = treeObj.get_family().values()
        return Response(response)

class GalleryTreeItemUserPermissionViewSet(viewsets.ModelViewSet):
    serializer_class = GalleryTreeItemUserPermissionSerializer
    ...
    def create(self, request, *args, **kwargs):
        response = add_gallery_user_permission(self, request.data)
        return Response(response)
    ...
    def list(self, request, *args, **kwargs):
        ...
        tree_item_data = {temp['user_id']: temp for temp in GalleryTreeItemUserPermission.objects.filter(user__in=user_ids,
...
class GalleryTreeItemGroupPermissionViewSet(viewsets.ModelViewSet):
    serializer_class = GalleryTreeItemGroupPermissionSerializer
    http_method_names = ['post', 'get']

    def get_queryset(self):
        self.queryset = GalleryTreeItemGroupPermission.objects.filter(tree_item=self.request.GET.get('tree_item'))
        return self.queryset

    def create(self, request, *args, **kwargs):
        response = add_gallery_group_permission(self, request.data)
        return Response(response)

    def list(self, request, *args, **kwargs):
        response = {'data': {'groups': []}}
        self.serializer_class = GalleryTreeItemGroupPermissionReadSerializer
        if not self.request.GET.get('tree_item'):
            raise exceptions.ValidationError('tree_item is mandatory')
...
class GalleryTreeItemStandardPermissionViewSet(viewsets.ModelViewSet):
    serializer_class = GalleryTreeItemStandardPermissionSerializer
    http_method_names = ['post', 'get']
    ...
    def get_queryset(self):
        if self.request.method == 'GET':
            ...
            self.queryset = GalleryTreeItemStandardPermission.objects.all()
        return self.queryset

    def create(self, request, *args, **kwargs):
        response = add_gallery_standard_permission(self, request.data)
        return Response(response)
    ...
    def list(self, request, *args, **kwargs):
        ...
        permission_data = {tree['standard_id']: tree for tree in GalleryTreeItemStandardPermission.objects.filter(standard__in=standard_ids,
...
class GalleryTreeItemStandardSectionPermissionViewSet(viewsets.ModelViewSet):
    serializer_class = GalleryTreeItemStandardSectionPermissionSerializer
    http_method_names = ['post', 'get']
    ...
    def get_queryset(self):
        if self.request.method == 'GET':
            ...
            self.queryset = GalleryTreeItemStandardSectionPermission.objects.all()
        return self.queryset

    def create(self, request, *args, **kwargs):
        response = add_gallery_standard_section_permission(self, request.data)
        return Response(response)
    ...
    def list(self, request, *args, **kwargs):
        ...
        if self.request.GET.get('get_academic_years'):
            return Response({'data': GalleryTreeItemStandardSectionPermission.objects.filter(
                tree_item=tree_item
        ...
        permission_data = {tree['standard_section_id']: tree for tree in GalleryTreeItemStandardSectionPermission.objects.filter(tree_item=tree_item, standard_section__in=standard_section_list).values()}
        for standard_section in standard_section_data['data']:
            for section in standard_section['sections']:
...
class GalleryCopyPermissionViewSet(viewsets.ModelViewSet):
