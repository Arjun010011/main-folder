...
from apps.gallery.models import GalleryFolder, GalleryFile, GalleryTreeItem,GallerySetup
...
from apps.gallery.serializers import GalleryFileSerializer, GalleryFolderSerializer
...
def create_folder(self, data):
    ...
    with transaction.atomic(using=get_current_db_name()):
        folderClass = ContentType.objects.get(model='GalleryFolder').model_class()
        serializer = GalleryFolderSerializer(data=data)
...
def validate_level_model(self, data): #planned for second phase
    ...
    try:
        obj = GallerySetup.objects.get(level=level)
    except:
        return
...
def rename_folder_or_file(self, data):
    ...
    if childObj.parent_id:
        parentObj = GalleryTreeItem.objects.get(id=childObj.parent_id)
        treeType = check_new_name_exist(self, data['name'], childObj,parentObj)
        if treeType == 'folder':
            GalleryFolder.objects.filter(id=childObj.object_id).update(name=data['name'])
        elif treeType == 'file':
            GalleryFile.objects.filter(id=childObj.object_id).update(name=data['name'])
        else:
...
def update_description(self, data):
    ...
    if childObj.content_type.model == 'folder':
        GalleryFolder.objects.filter(id=childObj.object_id).update(description=data['description'])
    elif childObj.content_type.model == 'file':
        GalleryFile.objects.filter(id=childObj.object_id).update(description=data['description'])
    return {'Reason' : 'Data updated Successfully'}
...
def create_file(self, data_set):
    ...
    allow_duplicate_files = FormdefinitionService.get_formdefintion_data(self, 'gallery_configuration', 'allow_duplicate_files')
    ...
    for data in row_data['data_list']:
        with transaction.atomic(using=get_current_db_name()):
            serializer = GalleryFileSerializer(data=data)
            serializer.is_valid(raise_exception=True)
            serializer = serializer.save()
...
def save_file(self, data):
    file_serializer = GalleryFileSerializer(data=data)
...
def check_for_child_permissions(self, tree_item_ids, user_obj, permission_tree_items_ids={}):
    ...
    if is_super_user:
        ...
        return temp
    user_permissions = GalleryTreeItemUserPermission.objects.filter(user=user_id, tree_item__in=temp_tree_item_ids).values(
        'tree_item', 'permission_mode'
    )
    tree_item_data = GalleryTreeItem.objects.filter(id__in=tree_item_ids)
    ...
    parent_ids = GalleryTreeItem.objects.filter(id__in=temp_tree_item_ids).values('parent_id','id')
    parent_id_list = []
    parent_dict ={}
    ...
    group_permissions = GalleryTreeItemGroupPermission.objects.filter(group=group_id, tree_item__in=temp_tree_item_ids+parent_id_list).values(
        'tree_item', 'permission_mode'
    )
    ...
    if not is_staff:
        ...
        if current_academic_year and current_standard:
            standard_permissions = GalleryTreeItemStandardPermission.objects.filter(
        ...
        if current_standard_section:
            standard_section_permissions = GalleryTreeItemStandardSectionPermission.objects.filter(tree_item__in=temp_tree_item_ids+parent_id_list, standard_section=current_standard_section).values('tree_item', 'permission_mode')
...
def get_bread_crums_for_folder(self, id):
    ...
    if int(id) and id:
        ...
        treeData = GalleryTreeItem.objects.filter(id__in=descendantIds).exclude(id=1)
        folderIds = []
        for row in treeData:
            if row.content_type.model == 'folder':
                folderIds.append(row.object_id)
        folders = GalleryFolder.objects.filter(id__in=folderIds).annotate(tree_id=F('tree_relation__id')).order_by(F('tree_relation__level')).values(*folderFilterValues)
    else:
        folders = []
    return folders
...
def get_folders_and_files(self, data, imageAndVideo=False):
    ...
    data = GalleryTreeItem.objects.filter(id__in=tree_item_ids)
    ...
    folders = GalleryFolder.objects.filter(id__in=folderIds)
    folderSerializer = GalleryFolderSerializer(folders, many=True)
    folders = folderSerializer.data
    files = GalleryFile.objects.filter(id__in=fileIds)
    fileserializer = GalleryFileSerializer(files, many=True)
...
def move_folder_or_file(self, data, childId):
    with transaction.atomic(using=get_current_db_name()):
        ...
        parentObj = GalleryTreeItem.objects.get(id=parentId)
        childObj = GalleryTreeItem.objects.get(id=childId)
...
def check_folder_or_file_already_exist(self, childObj, parentObj):
    ...
    if childObj.content_type.model == 'galleryfolder' and  self.get_queryset().filter(parent=parentObj,folder__name=childObj.folder.values()[0]['name'], is_active=True).exists():
        raise exceptions.ValidationError('Folder Name already exist')
    elif childObj.content_type.model == 'galleryfile' and self.get_queryset().filter(parent=parentObj,file__name=childObj.file.values()[0]['name'], is_active=True).exists():
...
def check_new_name_exist(self, newName, childObj, parentObj):
    if childObj.content_type.model == 'galleryfolder' and  self.get_queryset()\
        .filter(parent=parentObj,folder__name=newName, is_active=True).exclude(id=childObj.id).exists():
        raise exceptions.ValidationError('Folder Name already exist')
    elif childObj.content_type.model == 'galleryfile' and self.get_queryset()\
...
def read_folder_file_obj(self, tree_item_obj):
    obj = None
    file_type = ''
    if tree_item_obj.content_type.model == 'galleryfolder':
        obj = GalleryFolder.objects.filter(id=tree_item_obj.object_id)
        file_type = 'folder'
    elif tree_item_obj.content_type.model == 'galleryfile':
        obj = GalleryFile.objects.filter(id=tree_item_obj.object_id)
        file_type = 'file'
