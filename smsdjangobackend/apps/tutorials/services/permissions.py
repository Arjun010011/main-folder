...
from apps.tutorials.models.mptt import TreeItem, TreeItemGroupPermission, TreeItemStandardPermission, TreeItemStandardSectionPermission, TreeItemUserPermission
from django.db import transaction
from apps.tenants.services.middlewares import get_current_db_name
from apps.tutorials.serializers import TreeItemGroupPermissionSerializer, TreeItemSerializer, TreeItemStandardPermissionSerializer, TreeItemStandardSectionPermissionSerializer, TreeItemUserPermissionSerializer


def update_tree_item_data(self, data_to_update):
    for data in data_to_update:
        TreeItem.objects.filter(id=data['tree_item']).update(
            data
        )


def add_tutorial_user_permission(self, data):
    from apps.users.models import User
    tree_item_data = TreeItem.objects.filter(id__in=data['tree_item_list'])
    ...
    with transaction.atomic(using=get_current_db_name()):
        if deletable_ids:
            TreeItemUserPermission.objects.filter(id__in=deletable_ids).delete()
        if is_delete_existing_permission:
            TreeItemUserPermission.objects.filter(tree_item__in=data['tree_item_list']).delete()
        if data_to_save:
            for data in data_to_save:
                if 'id' in data and data['id']:
                    kwargs = {'customObjectData': TreeItemUserPermission.objects.get(id=data['id'])}
...
def add_tutorial_group_permission(self, data):
    tree_item_data = TreeItem.objects.filter(id__in=data['tree_item_list'])
    ...
    with transaction.atomic(using=get_current_db_name()):
        if deletable_ids:
            TreeItemGroupPermission.objects.filter(id__in=deletable_ids).delete()
        if is_delete_existing_permission:
            TreeItemGroupPermission.objects.filter(tree_item__in=data['tree_item_list']).delete()
        for data in data_to_save:
            if 'id' in data and data['id']:
                kwargs = {'customObjectData': TreeItemGroupPermission.objects.get(id=data['id'])}
                SharedService.update_data(self, data, **kwargs)
            else:
                SharedService.add_data(self, data, False)
        if data_to_update_tree_item:
...
def add_tutorial_standard_permission(self, data):
    tree_item_data = TreeItem.objects.filter(id__in=data['tree_item_list'])
    ...
    if not is_delete_existing_permission: #if existing is deleting no need to check the existing permission
        ...
        existing_data = TreeItemStandardPermission.objects.filter(standard__in=standard_ids).values()
    ...
    with transaction.atomic(using=get_current_db_name()):
        if deletable_ids:
            TreeItemStandardPermission.objects.filter(id__in=deletable_ids).delete()
        if is_delete_existing_permission:
            TreeItemStandardPermission.objects.filter(tree_item__in=data['tree_item_list']).delete()
        if data_to_save:
            for data in data_to_save:
                if 'id' in data and data['id']:
                    kwargs = {'customObjectData': TreeItemStandardPermission.objects.get(id=data['id'])}
                    SharedService.update_data(self, data, **kwargs)
                else:
                    SharedService.add_data(self, data, False)
        if data_to_update_tree_item:
...
def add_tutorial_standard_section_permission(self, data):
    tree_item_data = TreeItem.objects.filter(id__in=data['tree_item_list'])
    ...
    with transaction.atomic(using=get_current_db_name()):
        if deletable_ids:
            TreeItemStandardSectionPermission.objects.filter(id__in=deletable_ids).delete()
        if is_delete_existing_permission:
            TreeItemStandardSectionPermission.objects.filter(tree_item__in=data['tree_item_list']).delete()
        if data_to_save:
            for data in data_to_save:
                if 'id' in data and data['id']:
                    kwargs = {'customObjectData': TreeItemStandardSectionPermission.objects.get(id=data['id'])}
                    SharedService.update_data(self, data, **kwargs)
                else:
                    SharedService.add_data(self, data, False)
        if data_to_update_tree_item:
...
def get_user_permission_modes(self, tree_item_ids):
    ...
    if standard_id:
        standard_permission = {tree['tree_item'] : tree for tree in TreeItemStandardPermission.objects.filter(
            tree_item__in=tree_item_ids, standard=standard_id
        ).values('permission_mode', 'tree_item')}
    if standard_section_id:
        standard_section_permission = {tree['tree_item'] : tree for tree in TreeItemStandardSectionPermission.objects.filter(
            tree_item__in=tree_item_ids, standard_section=standard_section_id
        ).values('permission_mode', 'tree_item')}
    groups = self.request.user.groups.values_list('id',flat = True)
    if groups:
        group_permission = {tree['tree_item'] : tree for tree in TreeItemGroupPermission.objects.filter(tree_item__in=tree_item_ids, group__in=list(groups)).values('permission_mode', 'tree_item')}
    user_permission = {tree['tree_item'] : tree for tree in TreeItemUserPermission.objects.filter(tree_item__in=tree_item_ids, user=self.request.user.id).values('permission_mode', 'tree_item')}
    tree_item_data = {}
...
def copy_permission(self, data):
    copy_from_tree_obj = TreeItem.objects.get(id=data['copy_from_tree_id'])
    ...
    copy_to_tree_objs = TreeItem.objects.filter(id__in=data['copy_to_tree_ids']).values(*values)
    ...
    group_permission = list(TreeItemGroupPermission.objects.filter(tree_item=copy_from_tree_obj.id).values())
    user_permission = list(TreeItemUserPermission.objects.filter(tree_item=copy_from_tree_obj.id).values())
    standard_permission = list(TreeItemStandardPermission.objects.filter(tree_item=copy_from_tree_obj.id).values())
    standard_section_permission = list(TreeItemStandardSectionPermission.objects.filter(tree_item=copy_from_tree_obj.id).values())
    group_data_to_save = []
    user_data_to_save = []
    ...
    with transaction.atomic(using=get_current_db_name()):
        if group_data_to_save:
            TreeItemGroupPermission.objects.filter(tree_item__in=data['copy_to_tree_ids']).delete()
            serializer = TreeItemGroupPermissionSerializer(data=group_data_to_save, many=True)
            serializer.is_valid(raise_exception=True)
            serializer.save()
        if user_data_to_save:
            TreeItemUserPermission.objects.filter(tree_item__in=data['copy_to_tree_ids']).delete()
            serializer = TreeItemUserPermissionSerializer(data=user_data_to_save, many=True)
            serializer.is_valid(raise_exception=True)
            serializer.save()
        if standard_data_to_save:
            TreeItemStandardPermission.objects.filter(tree_item__in=data['copy_to_tree_ids']).delete()
            serializer = TreeItemStandardPermissionSerializer(data=standard_data_to_save, many=True)
            serializer.is_valid(raise_exception=True)
            serializer.save()
        if standard_section_data_to_save:
            TreeItemStandardSectionPermission.objects.filter(tree_item__in=data['copy_to_tree_ids']).delete()
            serializer = TreeItemStandardSectionPermissionSerializer(data=standard_section_data_to_save, many=True)
            serializer.is_valid(raise_exception=True)
            serializer.save()
