...
from apps.tutorials.models import TreeItem, Folder, File
...
class MPTTCustomAdmin(DraggableMPTTAdmin):
    ...
    def indented_title(self, item):
        ...
        """
        tempLabel = ''
        if item.content_type.model == 'folder':
            tempLabel = item.folder.values()[0]['name']+' (Folder)'
        elif item.content_type.model == 'file':
...
admin.site.register(TreeItem, MPTTCustomAdmin)# dont use this because folder can be inside file which is bug
