...
const fieldDetails = [
    ...
    {
        label: 'Max Amount', regex: amountRegexWithDecimals, autoFocus: false, name: 'max_amount', md: 6, className: 'width-80',
        required: false, id: 'outlined-textarea', default: '', rows: null, type: 'text', isDuplicateAllow: true,
        maxLength: 9
    },
]
...
class AddExpensesPlan extends Component {
    ...
    constructor(props) {
        ...
        this.viewUrl = Actions.hostel_expenses_plan.view.url
    }
    ...
    componentDidMount = () => {
        ...
        this.setState({
            year: year,
            yearName: yearName,
            header: `Add Hostel Expenses Plan for ${yearName}`
        })
    ...
    getExpensesTypes = (year) => {
        ...
        const params = { is_active: true, financial_year: year, expense_for: 2 }
        ...
        postRequest(url, post_data, this.props)
            .then((response) => {
                if (response && response.status === 200) {
                    ...
                    this.props.history.push(Actions.hostel_expenses_plan.view.url)
                }
