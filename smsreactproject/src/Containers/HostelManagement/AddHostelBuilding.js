...
class AddHostel extends Component {
    ...
    componentDidMount = () => {
        if (this.props.location.pathname === Actions.hostel.update.url) {
    ...
    updateAllDetails = (id) => {
        ...
        hostel['building_for'] = this.getHostelForAndGender(hostel_details.building_for)
    ...
    validation = () => {
        ...
        if (!hostel.hostelFor) {
            fieldErrors['hostelFor'] = 'Select for'
        }
        ...
        if (returnValue) {
            ...
            temp['building_type'] = 'Hostel'
            temp['building_for'] = this.getHostelFor()
    ...
    render() {
        const { loading, hostel, fieldErrors, openError, alertData, submitDisable, genderList, hostelForList, updatingDropDown } = this.state
        if (loading) {
            return (
                <Box display='flex'>
                                        ...
                                        <Grid container spacing={2} className=''>
                                            ...
                                            <Grid item md={6} xs={12}>
                                                <Dropdown
                                                    data={hostelForList}
                                                    name='hostelFor'
                                                    value={hostel.hostelFor}
                                                    className='width-100'
                                                    onChange={(e) => this.handleSearchChange(e)}
                                                    error={fieldErrors['hostelFor']}
                                                    label='For'
                                                    hideSelect={true}
                                                />
                                            </Grid>
                                            <Grid item md={6} xs={12}>
                                                {hostel.hostelFor == 1 &&
                                                    <Dropdown
                                                        ...
                                                        value={hostel.gender}
                                                        className='width-100'
                                                        disabled={hostel.hostelFor ? false : true}
                                                        onChange={(e) => this.handleSearchChange(e)}
                                                        error={fieldErrors['gender']}
                                                        label='Gender'
                                                        hideSelect={true}
                                                        helperText={hostel.hostelFor ? '' : 'Select hostel for first'}
                                                        customName='name'
                                                    />
                                                }
                                                {(hostel.hostelFor == 2 || hostel.hostelFor == 3) &&
                                                    <Dropdown
                                                        data={genderList}
                                                        name='gender'
                                                        value={hostel.gender}
...
export default withRouter(AddHostel)