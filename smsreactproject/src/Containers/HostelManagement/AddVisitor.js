...
import AddStudentOrStaff from "./Components/AddStudentOrStaff";
...
class AddVisitor extends Component {
  constructor(props) {
    ...
    this.state = {
      visitor: { name: "", checkIn: null, checkOut: null },
      fieldErrors: {},
      helperText: {},
      loading: true,
      openError: false,
  ...
  componentDidMount = () => {
    this.getReasonList();
    if (this.props.location.pathname === Actions.hostel_visitor.update.url) {
      if (this.props.location.state && this.props.location.state.detail) {
        ...
        this.props.history.push(Actions.hostel_visitor.view.url);
      }
    } else {
  ...
  getReasonList = () => {
    ...
    const params = { is_active: true, reason_type: reasonType["hostel"] };
  ...
  updateAllDetails = (id) => {
    ...
    if (hostel_details.allocation_details) {
      data_details["selected_name"] =
        hostel_details.allocation_details &&
        hostel_details.allocation_details.staff
          ? "staff"
          : "student";
    }
    if (
      ...
      data_details["details"]["full_name"] =
        hostel_details.allocation_details.staff_details.full_name;
      data_details["details"]["mobile_num"] =
        hostel_details.allocation_details.staff_details.mobile_num;
      data_details["details"]["email"] =
        hostel_details.allocation_details.staff_details.email;
      data_details["details"]["group_name"][0] =
        hostel_details.allocation_details.staff_details.group_name[0];
      data_details["details"]["roomallocation_staff"]["building_name"] =
        hostel_details.allocation_details.building_name;
      data_details["details"]["roomallocation_staff"]["floor_name"] =
        hostel_details.allocation_details.floor_name;
      data_details["details"]["roomallocation_staff"]["room_name"] =
        hostel_details.allocation_details.room_name;
      data_details["details"]["roomallocation_staff"]["id"] =
        hostel_details.allocation_details.id;
    } else if (
      data_details["selected_name"] === "student" &&
      hostel_details.allocation_details
    ) {
      data_details["details"]["name"] =
        hostel_details.allocation_details.student_details.name;
      data_details["details"]["standard"] =
        hostel_details.allocation_details.student_details.current_standard_name;
      data_details["details"]["mobile_num"] =
        hostel_details.allocation_details.student_details.mobile_num;
      data_details["details"]["email"] =
        hostel_details.allocation_details.student_details.email;
      data_details["details"]["roomallocation_student"]["building_name"] =
        hostel_details.allocation_details.building_name;
      data_details["details"]["roomallocation_student"]["floor_name"] =
        hostel_details.allocation_details.floor_name;
      data_details["details"]["roomallocation_student"]["room_name"] =
        hostel_details.allocation_details.room_name;
      data_details["details"]["roomallocation_student"]["id"] =
        hostel_details.allocation_details.id;
    }
  ...
  validation = () => {
    ...
    if (returnValue) {
      ...
      let room_allocation_id = "";
      if (data_details.selected_name === "staff") {
        room_allocation_id = data_details.details.roomallocation_staff.id;
      } else if (data_details.selected_name === "student") {
        room_allocation_id = data_details.details.roomallocation_student.id;
      }

      returnValue["name"] = visitor.name;
      returnValue["checkin"] = dateFormat(
        visitor.checkIn,
        "YYYY-MM-DD HH:mm:ss"
      );
      returnValue["checkout"] = visitor.checkOut
        ? dateFormat(visitor.checkOut, "YYYY-MM-DD HH:mm:ss")
        : null;
      returnValue["reason"] = visitor.reason && visitor.reason["id"];
      returnValue["building"] = selectedBuilding;
      returnValue["roomallocation"] = room_allocation_id;
    } else {
      openError = true;
    }
    this.setState({
  ...
  submit = () => {
    ...
    if (validate_post_format) {
      ...
      postRequest(url, bodyFormData, this.props).then((response) => {
        if (response && response.status === 200) {
          Swal.fire({
            position: "top-end",
            type: "success",
  ...
  gotoViewVisitors = () => {
    ...
    this.props.history.push({
      pathname: Actions.hostel_visitor.view.url,
      search: searchParam,
    });
  };
  ...
  updatePostFormat = (newData) => {
    ...
    newData.reason_type = reasonType["hostel"];
    let payload = {
      reason: [newData],
    };
    return payload;
          ...
          <Paper className="paper-background">
            <Grid container>
              <Grid item md={8} xs={12} className="header-align">
                <Box className="heading">Hostel Visitor</Box>
              </Grid>
              <Grid item md={4} xs={12}>
                <Box className="header-align end-flex-prop">
                  {isUserHasPermission("hostel_visitor", "view") && (
                    <Button
                      variant="contained"
                      onClick={() => this.gotoViewVisitors()}
                      className="editbutton-view"
                    >
                      <VisibilityOutlinedIcon className="visibility-icon" />{" "}
                      {Actions.hostel_visitor.view.label}
                    </Button>
                  )}
                </Box>
              </Grid>
                  ...
                  <Paper className="paper-plain-background header-align p-b-20px">
                      ...
                      <Grid item md={6} xs={12}>
                        <MuiPickersUtilsProvider utils={DateFnsUtils}>
                          <KeyboardDateTimePicker
                            ...
                            onChange={(e) =>
                              this.handleCheckInOut(e, "checkIn")
                            }
                            KeyboardButtonProps={{
                      ...
                      <Grid item md={6} xs={12}>
                        <MuiPickersUtilsProvider utils={DateFnsUtils}>
                          <KeyboardDateTimePicker
                            ...
                            onChange={(e) =>
                              this.handleCheckInOut(e, "checkOut")
                            }
                            KeyboardButtonProps={{
                              "aria-label": "change date",
                            }}
                      ...
                      {!loadingOptions && (
                        <Grid item md={6} xs={12} className="mt-20">
                          <DropDownWithSearchAndAddApi
                            ...
                            className="width-90"
                            helperText={
                              visitor.reason ? `` : fieldErrors["reason"]
                            }
                    ...
                    {/* <Grid container>
                                            <Grid item md={12}>
                                                ...
                                                >
                                                    <Box className='create-hostel-building-address-label header-align'>Reason *</Box>
 ...
 */}
                ...
                <Grid item md={4} xs={12}>
                  ...
                  <Paper
                    className={
                      data_details.selected_name ? "m-t-20px" : "display-none"
                    }
                  >
                    {data_details.selected_name && (
                    ...
                    {data_details.selected_name === "staff" && (
                      <Box className="header-align">
                        ...
                        <Box className="create-expenses-outer-box-label-value">
                          ...
                          <Box className="create-expenses-value">
                            {
                              data_details.details.roomallocation_staff
                                .building_name
                            }
                          </Box>
                        </Box>
                        <Box className="create-expenses-outer-box-label-value">
                          ...
                          <Box className="create-expenses-value">
                            {`${data_details.details.roomallocation_staff.floor_name} (${data_details.details.roomallocation_staff.room_name})`}
                          </Box>
                        </Box>
                      </Box>
                    )}
                    {data_details.selected_name === "student" && (
                      <Box className="header-align">
                        ...
                        <Box className="create-expenses-outer-box-label-value">
                          ...
                          <Box className="create-expenses-value">
                            {
                              data_details.details.roomallocation_student
                                .building_name
                            }
                          </Box>
                        </Box>
                        <Box className="create-expenses-outer-box-label-value">
                          ...
                          <Box className="create-expenses-value">
                            {`${data_details.details.roomallocation_student.floor_name} (${data_details.details.roomallocation_student.room_name})`}
                          </Box>
                        </Box>
                      </Box>
                    )}
...
export default withRouter(AddVisitor);
