...
const EvaluateStudentQuiz = React.forwardRef((props, ref) => {
    ...
    const getQuestionDetails = () => {
        ...
        const url = GET_URL.forms.api + form_code + '/'
    ...
    const goToReviewPage = () => {
        if (quizDetails.form_standard_section_mapping_form) {
            ...
            props.history.push({
                pathname: Actions.set_quiz.update.url,
                search: searchParam,
    ...
    const saveData = (name) => {
        ...
        if (validate) {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    let url = PUT_URL.evaluate.api + quizDetails['response_data']['id'] + '/';
...
export default withRouter(EvaluateStudentQuiz)