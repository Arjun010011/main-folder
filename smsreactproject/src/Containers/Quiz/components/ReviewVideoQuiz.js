...
import ViewStudentStaffTableQuiz from 'Containers/Quiz/components/ViewStudentStaffTableQuiz';
...
const ReviewVideoQuiz = React.forwardRef((props, ref) => {

    const { standard_name, openReviewPage, questionList, review_details, year, loadingReviewDetails } = props;
    ...
    const getQuestions = () => {
        ...
        questionList.map((data, index) => {
            ...
            question_temp['time_limit_to_answer'] = review_details.is_total_time === 'yes' ? 0 : data.question_details.withinTime
            question_temp['show_answer_after_submit'] = data.question_details.showAnswer === 'yes' ? true : false
            question_temp['question_start_time'] = data.playedSeconds
    ...
    const getStaffList = () => {
        let staffs = []
        let staff_temp = {}
        review_details.staff_list.map((data) => {
            staff_temp = {}
            staff_temp['staff'] = data.staff ? data.staff : data.id
            staff_temp['view'] = data.view
            staff_temp['update'] = data.update
            staff_temp['evaluate'] = data.evaluate
            staffs.push(staff_temp)
        })
        return staffs
    }
    ...
    const getPostFormat = () => {
        let post_data = {
            "form": {
                "title": review_details.quiz_title,
                ...
                "standard_section_ids": getSectionList(),
                "students": getStudentList(),
                "alternate_teachers": getStaffList(),
                "questions": getQuestions(),
                "is_automatic_evaluation": review_details.is_automatic_evaluation,
                'is_questions_at_end': review_details.is_questions_at_end,
                'is_video_quiz': true,
                'document': review_details.video_id,
            }
        }
                            ...
                            return (<Box className='m-t-20px' key={index}>
                                ...
                                <Box display='flex'>
                                    ...
                                    <Box className='quiz-review-right-set'>
                                        <Box display='flex'>
                                            <Box className='quiz-review-label'>Question At Time :</Box>
                                            <Box className='quiz-review-value'>{getTimeFormatFromSeconds(data.playedSeconds)}</Box>
                                        </Box>
                                        <Box display='flex'>
...
export default ReviewVideoQuiz