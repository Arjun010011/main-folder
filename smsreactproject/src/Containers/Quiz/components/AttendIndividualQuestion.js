...
class AttendIndividualQuestion extends Component {
  constructor(props) {
    super(props);
  ...
  handleClose = () => {
    ...
    this.props.history.push(Actions.attend_quiz_list.view.url);
  ...
  handleSubmit = (name, sequenceTemp) => {
    ...
    if (validate_post_data && validate_post_data !== "dontSubmit") {
      const url = POST_URL.response.api;
  ...
  getQuestions = (sequence, name) => {
    ...
    if (validate) {
      ...
      const url = GET_URL.forms.api + selectedQuiz + "/";
        ...
        >
          <AppBar>
            <Toolbar>
              ...
              <Typography variant="h6">Quiz Title - {quizName}</Typography>
            </Toolbar>
          </AppBar>
          {loadingDetails ? (
            <LoadingGif />
                          ...
                          <Grid container>
                            ...
                            <Grid item md={4} xs={12}>
                              <Box display="flex">
                                ...
                                <Box
                                  className={
                                    questions.time_limit_to_answer &&
                                    parseInt(seconds) < 10
                                      ? "breathing-button quiz-review-value red-text font-weight-bold fs-18"
                                      : "quiz-review-value red-text  font-weight-bold fs-18"
                                  }
                                >
                                  {!questions.time_limit_to_answer
                                    ? "---"
                                    : questions.time_limit_to_answer &&
                                      getTimeFormatFromSeconds(seconds)}
                                </Box>
                              </Box>
                              <Box display="flex">
                                <Box className="quiz-review-label">
...
export default withRouter(AttendIndividualQuestion);
