...
class ManageDepartmentView extends Component {
  constructor() {
    super();
    this.permission = updatePermissions('department', ['update', 'delete']);
            ...
            customBodyRender: (value, tableMeta) => {
              return (
                <ActionColumn
                  ...
                  updateUrl={PUT_URL.branch.api}
                  updatePostFormat={this.updatePostFormat}
                  updateType={this.updateType}
                  deleteUrl={DEL_URL.branch.api}
          ...
          <Grid container>
            ...
            <Grid item md={6} xs={12}>
              <Box className={classNames('header-align', 'end-flex-prop')}>
                {isUserHasPermission('department', 'create') && (
                  <Button
                    variant="contained"
                    component={Link}
                    to={Actions.departmentdata.create.url}
                    className="editbutton-view"
                  >
                    <AddCircleOutlineOutlinedIcon className="visibility-icon" />{' '}
                    {Actions.departmentdata.create.label}
                  </Button>
                )}
              </Box>
            </Grid>
