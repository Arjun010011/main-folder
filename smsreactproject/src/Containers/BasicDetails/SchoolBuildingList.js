...
import { isUserHasPermission, getFinancialYear, SetFinancialYear, numberWithCommas, dateFormat } from 'Includes/functions';
import { options } from 'Constants';
import { values } from 'react-intl/locale-data/hi';

class SchoolBuildingList extends Component {
    constructor() {
        super()
        this.state = {
            hostelBuildingList: [],
                        ...
                        customBodyRender: (value, tableMeta, updateValue) => {
                            return (<div>
                                <StudentListActions
                                    ...
                                    editURL={Actions.school_building.update.url}
                                    viewURL={Actions.building_individual.view.url}
    ...
    updatePermissions = (name) => {
        let test = true
        const hasViewPermission = isUserHasPermission('building_individual', 'view')
        const hasEditPermission = isUserHasPermission('school_building', 'update')
        const hasDeletePermission = isUserHasPermission('school_building', 'delete')
        let enabledActions = [];
    ...
    handleAddExpensesButton = () => {
        this.props.history.push({
            pathname: Actions.school_building.create.url,
        });
    }
                    ...
                    <Paper className={classNamePaper}>
                        <Grid container>
                            <Grid item md={6} xs={12} className={classNames('header-align')}>
                                ...
                                {Actions.school_building.view.label}
                                </Box>
                            </Grid>

                            <Grid item md={6} xs={12} >
                                <Box className={classNames('header-align', 'end-flex-prop')}>
                                    {isUserHasPermission('school_building', 'create') && <Button
                                        variant="contained"
                                        onClick={this.handleAddExpensesButton}
                                        className='editbutton-view'
                                    ><AddCircleOutlineOutlinedIcon className='visibility-icon' /> {Actions.school_building.create.label}</Button>
                                    }
                                </Box>
...
export default withRouter(SchoolBuildingList)
