...
class SchoolAddressIndividualView extends Component {
    ...
    updateExpenseView = () => {
        ...
        let shift = {
            'sub_heading': 'Address Informations',
            'data':
                [{ label: <FormattedMessage {...commonMessages.address1} />, value: buildingDetails['map_address_data']?.['address_one_map'], md: 6 },
                { label: <FormattedMessage {...commonMessages.address2} />, value: buildingDetails['map_address_data']?.['address_two_map'], md: 6 },
                { label: <FormattedMessage {...commonMessages.city} />, value: buildingDetails['map_address_data']?.['city_map'] },
                { label: <FormattedMessage {...commonMessages.district} />, value: buildingDetails['map_address_data']?.['district_map'] },
                { label: <FormattedMessage {...commonMessages.state} />, value: buildingDetails['map_address_data']?.['state_map'] },
                { label: <FormattedMessage {...commonMessages.country} />, value: buildingDetails['map_address_data']?.['country_map'] },
                { label: <FormattedMessage {...commonMessages.pincode} />, value: buildingDetails['map_address_data']?.['pincode_map'] }]
        }
        building_details.push(shift);
        shift = {
            'sub_heading': 'Standards',
            'data':[]
        }
        let standards=[]
        buildingDetails.standard_data.map((data)=>{
            standards.push(data['name'])
        })
        shift['data']=[{label:'',value: standards.join(`, `),md: 12}]
        building_details.push(shift);
    ...
    handleEdit = () => {
        ...
        this.props.history.push({
            pathname: Actions.school_address.update.url,
            ...
            return (
                <Paper className='paper-background'>
                    <Grid container>
                        <Grid item md={8} xs={12} className='header-align'>
                            <Box className='heading'>
                                Institute Address Detail
                            </Box> 
                        </Grid>
                        <Grid item md={4} xs={12} >
                            <Box className='header-align end-flex-prop'>
                                {isUserHasPermission('school_address', 'view') &&
                                    <Button
                                        variant="contained"
                    ...
                    <Paper className='header-align expense-individual-paper-background'>
                        ...
                        {isUserHasPermission('school_address', 'edit') &&
...
export default withRouter(withStyles(Styles)(SchoolAddressIndividualView));