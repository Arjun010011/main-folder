...
class MarksCardGeneratorV2 extends Component {
  constructor(props) {
    super(props);
    this.state = {
      yearList: [],
      ...
      is_longrunning: isFormDefinitionEnabled(
        "exam_configurations",
        "is_marks_card_longrunning",
        1
      ),
    };
      ...
      if (value !== 0) {
        if (name === "selectedYear") {
          this.setState(
            ...
            () => {
              if (this.StudentHierarchy.current) {
  ...
  handleHallTicketDownload = (name, id, standard_sec_id, isConsolidated) => {
    ...
    let url = GET_URL.studentmarkv2.api;
    ...
    if (currentTab === 2 && typeWise === 1) {
      url = GET_URL.announceexamresultconfigv2.api;
      extra_param = { ...extra_param, ...{ academic_year: selectedYear } };
    } else if (currentTab === 2 && typeWise === 2) {
      url = GET_URL.studentmarkresultconfigv2.api;
      extra_param = { ...extra_param, ...{ academic_year: selectedYear } };
    } else if (currentTab === 2 && typeWise === 3) {
      url = GET_URL.studentmarkresultfinalconfigv2.api;
      extra_param = {
        ...extra_param,
        ...{ academic_year: selectedYear, exam: selectedExam },
      };
    }
    if (isConsolidated) {
      if (typeWise === 3) {
      ...
      if (
        currentTab === 1 ||
        (currentTab === 2 && typeWise === 3) ||
        (currentTab === 2 && typeWise === 2)
      ) {
...
export default withRouter(MarksCardGeneratorV2);
