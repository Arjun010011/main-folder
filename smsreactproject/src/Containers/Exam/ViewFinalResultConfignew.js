...
function ViewFinalResultConfignew(props) {
  ...
  const getExamList = (year) => {
    let yearId = selectedYear;
    if(year){
      yearId=year;
    }
    const url = GET_URL.exam.api;
    const param = { academic_year: selectedYear,is_active: true };
    getRequest(url, param, props).then((response) => {
      if (response && response.status === 200) {
  ...
  const handleChange = (e) => {
    ...
    if (name === "selectedYear") {
      ...
      getExamStandardList(selectedYear, selectedExam,value);
      SetStandard(value)
    }

  };

  const getExamStandardList = (yearId, examId,standardId) => {
    let url = GET_URL.examfinalresultconfigurationnew.api;
    let param = {
      is_active: true,
  ...
  const handleClickEnter = (standard, section) => {
    ...
    let pathName = Actions.final_result_config_new.create.url;
    props.history.push({
      pathname: pathName,
      search: searchParam,
    });
        ...
        <Paper className="paper-background">
          <Grid container>
            <Grid item md={6} xs={12} className="header-align">
              <Box className="heading">Exams Result Configuration</Box>
            </Grid>
          </Grid>
...
export default withRouter(ViewFinalResultConfignew);
