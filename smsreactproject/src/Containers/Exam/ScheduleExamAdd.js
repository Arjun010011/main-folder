...
class ScheduleExamAdd extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      multiSubjectList:[],
      standard_section: "",
      attendanceMode: "manual", // manual | auto
      fromDate: null,
      toDate: null,
  ...
  async componentDidMount() {
    let {
      ...
      end_date,
      selectedTerm,
      selectedStandard: selectedStandardFromUrl,
      standardName: standardNameFromUrl,
      termName,
    } = getUrlParam();
    const initialStandard = selectedStandardFromUrl || "";
    this.setState(
      {
        yearName,
        selectedYear,
        ...
        end_date,
        selectedTerm,
        selectedStandard: initialStandard,
        standardName: standardNameFromUrl || "",
        termName,
      },
      async () => {
        await this.loadStandardOptionsForExam(selectedExam);
        await this.loadScheduleForExam(selectedExam, this.state.selectedStandard || initialStandard);
      }
    );
  }

  loadStandardOptionsForExam = async (selectedExam) => {
    if (!selectedExam) return;
    const url = `${GET_URL.exam.api}${selectedExam}/`;
    const res = await getRequest(url, {}, { ...this.props, return_error: true });
    if (!(res && res.status === 200)) return;
    const data = res.data?.data || {};
    const standards = Array.isArray(data.standard_names) ? data.standard_names : [];
    const standardOptions = standards
      .map((s) => ({
        id: String(s?.standard ?? s?.standard_id ?? s?.id ?? ""),
    ...
    this.setState((prev) => {
      const keepSelected =
        prev.selectedStandard &&
        standardOptions.some((s) => String(s.id) === String(prev.selectedStandard));
      const fallback = standardOptions[0];
      return {
        standardOptions,
        selectedStandard: keepSelected ? prev.selectedStandard : fallback.id,
        standardName: keepSelected
          ? prev.standardName || standardOptions.find((s) => String(s.id) === String(prev.selectedStandard))?.name || ""
          : fallback.name,
      };
    });
  };

  loadScheduleForExam = async (selectedExam, selectedStandard = "") => {
    try {
      if (!selectedExam) {
        this.setState({
          loading: false,
          openError: true,
          alertData: "Exam details are missing. Please open schedule from exam list.",
        });
        return;
      }
      let param = { is_active: true, exam: selectedExam };
      if (selectedStandard) {
        param = { ...param, standard: selectedStandard };
      }
      ...
      this.setState({
        loading: false,
        openError: true,
        alertData: (error && error.message) || "Unable to load schedule details.",
      });
    }
  };

  handleScheduleStandardChange = (e) => {
    const selectedStandard = e.target.value;
    const { standardOptions, selectedExam } = this.state;
    const opt = (standardOptions || []).find((x) => String(x.id) === String(selectedStandard));
    this.setState(
      {
        selectedStandard,
        standardName: opt?.name || "",
        loading: true,
      },
      () => this.loadScheduleForExam(selectedExam, selectedStandard)
    );
  };
  ...
  setDefaultStandardList = (response) => {
    ...
    const stdMap = {};
    (getList || []).forEach((s) => {
      const sid = String(s.standard ?? "");
      if (!sid || stdMap[sid]) return;
      stdMap[sid] = {
        id: sid,
        name: s.standard_name || `Standard ${sid}`,
      };
    });
    const standardOptions = Object.values(stdMap);
    this.setState(
      (prev) => {
        const mergedMap = {};
        (prev.standardOptions || []).forEach((s) => {
          if (s && s.id) mergedMap[String(s.id)] = s;
        });
        (standardOptions || []).forEach((s) => {
          if (s && s.id) mergedMap[String(s.id)] = s;
        });
        const mergedStandardOptions = Object.values(mergedMap);
        return {
        ...
        part_type,
        section_list,
        standardOptions: mergedStandardOptions.length ? mergedStandardOptions : prev.standardOptions,
      };
      },
      () => {
        if (is_merge_subject) {
        ...
        if (
         ...
         if (show_manual_attendance_in_schedule) {
          standard.max_no_of_days_attendance = parseFloat(
            ...
            if (this.state.attendanceMode === "auto") {
              standard.attendance_from_date = this.state.fromDate;
              standard.attendance_to_date = this.state.toDate;
}
        if (
          is_grade_plan &&
          !selectedTotalGradePlan &&
      ...
      return (
        <div>
          <Paper className="paper-background">
            ...
            >
              <Grid container spacing={1.25}>
                <Grid item xs={12} sm={6} md={3}>
                  <Typography variant="caption" style={{ color: "#2563eb", fontWeight: 700 }}>Academic year</Typography>
                  <Box className="exam-mark-add-heading-bg">{yearName}</Box>
                </Grid>
                <Grid item xs={12} sm={6} md={3}>
                  <Typography variant="caption" style={{ color: "#2563eb", fontWeight: 700 }}>From</Typography>
                  <Box className="exam-mark-add-heading-bg">{dateFormat(start_date, "DD-MM-YYYY")}</Box>
                </Grid>
                <Grid item xs={12} sm={6} md={3}>
                  <Typography variant="caption" style={{ color: "#2563eb", fontWeight: 700 }}>To</Typography>
                  <Box className="exam-mark-add-heading-bg">{dateFormat(end_date, "DD-MM-YYYY")}</Box>
                </Grid>
                <Grid item xs={12} sm={6} md={3}>
                  <Typography variant="caption" style={{ color: "#2563eb", fontWeight: 700 }}>Term</Typography>
                  <Box className="exam-mark-add-heading-bg">{termName}</Box>
                </Grid>
                <Grid item xs={12} md={6}>
                  <Typography variant="caption" color="textSecondary">Exam</Typography>
                  <Box className="exam-mark-add-heading-bg">{examName}</Box>
                </Grid>
                {this.state.standardOptions && this.state.standardOptions.length > 0 && (
                  <Grid item xs={12}>
                    <Typography variant="caption" style={{ color: "#6366f1", fontWeight: 700 }}>
                      {`${alias_names["standard"] || "Standard"}`}
                    </Typography>
                    <Dropdown
                      ...
                      onChange={this.handleScheduleStandardChange}
                      label=""
                      customName="name"
                      customId="id"
                      hideSelect
            ...
            <Box
              className="margin-top-20 display-flex justify-content-space-between"
              p={1.5}
              mb={1}
              style={{
            ...
            >
              ...
              <div className="mt-10 d-flex" style={{ gap: 10, flexWrap: "wrap" }}>
                {is_grade_plan && (
                  <Paper
                    elevation={0}
                    className="mr-10 height-fit-content"
                    style={{ borderRadius: 10, padding: 4, border: "1px solid #e2e8f0" }}
                  >
                    <Dropdown
                      data={gradePlanList}
                ...
                {is_grade_plan && (
                  <Paper
                    elevation={0}
                    className="ml-10 height-fit-content"
                    style={{ borderRadius: 10, padding: 4, border: "1px solid #e2e8f0" }}
                  >
                    <Dropdown
                      data={gradePlanList}
            ...
            >
              <TextField
                ...
                className="width-300px"
                margin="normal"
                value={this.state.attendanceMode}
                onChange={(e) => this.setState({ attendanceMode: e.target.value })}
                size="small"
              >
                <MenuItem value="manual">Manual Attendance</MenuItem>
                <MenuItem value="auto">Auto Attendance</MenuItem>
              </TextField>

              {this.state.attendanceMode === "manual" && show_manual_attendance_in_schedule && (
                <TextField
                  variant="outlined"
                  id="max_no_of_days_attendance"
                  name="max_no_of_days_attendance"
                  label="Max No of days attendance"
                  className="width-300px ml-15"
                  margin="normal"
                  autoComplete="off"
                  value={max_no_of_days_attendance}
                  onChange={(e) => this.onChange(e)}
                  size="small"
                  error={Boolean(fieldError.max_no_of_days_attendance)}
                  helperText={fieldError.max_no_of_days_attendance}
                />
              )}

              {this.state.attendanceMode === "auto" && (
                <>
                  <TextField
                    label="From Date"
                    type="date"
