...
import TermWiseFinalResultConfig from "./components/TermWiseFinalResultConfig";
...
function AddFinalResultConfig(props) {
  ...
  const goToViewPage = () => {
    ...
    props.history.push({
      pathname: Actions.final_result_config.view.url,
      search: searchParam,
    });
  };
  ...
  React.useEffect(() => {
    const { year_name, standard_name, section_name, selectedYear } = getUrlParam();
    let examInformation = {
      year_name,
      standard_name,
      section_name,
          ...
          <Grid item md={6} xs={12}>
            <Box className="header-align end-flex-prop">
              ...
              >
                ...
                {Actions.final_result_config.view.label}
              </Button>
            </Box>
          </Grid>
        </Grid>
...
export default withRouter(AddFinalResultConfig);
