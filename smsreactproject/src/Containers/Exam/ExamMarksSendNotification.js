...
import ShowPreviewTemplate from "Containers/General/Components/ShowPreviewTemplate";

import ReactQuill from "react-quill";
import "react-quill/dist/quill.snow.css";

import PreviewUsers from "Containers/General/Components/PreviewUsers";

const DEFAULT_EXAM_RESULT_MESSAGE = `EXAM RESULT<br/>
{institute_name}<br/>
<br/>
Dear Parent,<br/>
<br/>
Your Ward {student_name},<br/>
<br/>
{exam_name} results are out.<br/>
<br/>
{subject_list_str}<br/>
Total Marks - {total_obtained_marks}<br/>
Rank - {standard_rank}<br/>
Result - {total_result}<br/>
<br/>
To View All Exam Marks Click Here : https://app.class10x.in/LMS/dashboard<br/>
<br/>
Note: NA means “Not Applicable”.<br/>
<br/>
Thanks and Regards<br/>
Principal, {institute_name}`;
...
class NotificationCreate extends Component {
  constructor() {
    ...
    this.state = {
      ...
      formDetails: {
        notification_medium: [],
        message: DEFAULT_EXAM_RESULT_MESSAGE,
        title: "",
        schedule_date_time: "",
        select_to_all: false,
        schedule_type: "now",
  ...
  componentDidMount = async () => {
    ...
    const { selectedExam, selectedTerm, selectedYear, selectedExamName } = locationState;
    try {
      const user = JSON.parse(localStorage.getItem("user"));
      const academic_year = selectedYear || user?.other_details?.academic_year?.id;
      this.setState(
        {
        ...
        () => {
          if (selectedExam) this.getExamSectionOptions(selectedExam);
        }
      );
  ...
  getMediumList = (response) => {
    let notificationMediumList = response.data.data.filter(
      (medium) => medium.name !== "ivr"
    );
    let mediumKeyValue = getKeyValueMap(notificationMediumList, "id", "name");
    this.setState({ notificationMediumList, mediumKeyValue });
  };

  getExamSectionOptions = (examId) => {
    if (!examId) return;
    this.setState({ loadingSections: true });
    const params = { is_active: true, exam: examId };
    getRequest(GET_URL.studentmarkclasssummary.api, params, this.props).then(
      (response) => {
        if (response && response.status === 200 && response.data.data) {
          const flatOptions = [];
          response.data.data.forEach((standard) => {
            const standardId = standard.id || standard.standard_id;
            const standardName = standard.standard__name || standard.name || "";
            (standard.section_list || []).forEach((section) => {
              flatOptions.push({
                id: section.id,
                standard_section: section.id,
                name: `${standardName} - ${section.section__name || section.section_name || ""}`,
                standard_id: standardId,
                standard_name: standardName,
                section_name: section.section__name || section.section_name || "",
              });
            });
          });
          this.setState({
            examSectionOptions: flatOptions,
      ...
      this.setState({ loadingSections: false, examSectionOptions: [] });
    });
  };

  onStandardSectionChange = (selectedList) => {
    const selectedSectionOptions = selectedList || [];
    const byStandard = {};
    selectedSectionOptions.forEach((opt) => {
      const sid = opt.standard_id;
      if (!byStandard[sid]) {
        byStandard[sid] = {
          id: sid,
      ...
      byStandard[sid].sections.push({
        standard_section: opt.id,
        id: opt.id,
        name: opt.section_name,
      });
    });
    const updated_list = Object.values(byStandard).map((item) => ({
      ...item,
      name: `${item.name} [${item.sections.map((s) => s.name).join(", ")}]`,
    }));
    this.setState({
      selectedSectionOptions: selectedSectionOptions,
      selected_details: {
        ...this.state.selected_details,
        updated_list,
      },
      selected_type: "section",
      isEditReceiver: updated_list.length > 0,
      fieldError: { ...this.state.fieldError, standard_section_ids: undefined },
    });
  };
  ...
  postApiCall = (validate) => {
    let url = POST_URL.studentmarkbulknotification.api;
  ...
  validateFields = (isApproved) => {
    ...
    if (
      selected_type === "section" &&
      (!selected_details["updated_list"] || selected_details["updated_list"].length === 0)
    ) {
      fieldError["standard_section_ids"] = "Please select at least one Standard Section";
    }
          ...
          <Paper className="mt-20 height-75vh p-20">
            <Grid container spacing={2} className="mt-20">
              ...
              <Grid item md={4} xs={12}>
                ...
                <div className="mt-20">
                  <div className="text-blue fs-18">Standard Section</div>
                  {loadingSections ? (
                    ...
                    <MultipleSelectDropdown
                      data_list={examSectionOptions}
                      selected_list={selectedSectionOptions}
                      onChange={this.onStandardSectionChange}
                      label="Select Standard Section"
                      customId="id"
                      optionValue="name"
                      className="width-100 mt-10"
                      error={fieldError["standard_section_ids"]}
                      disabled={!this.state.selectedExam}
                      helperText={!this.state.selectedExam ? "Go to Exam Marks, select an exam, then click Send Notification" : ""}
                    />
                  )}
                </div>
            ...
            <Box className="submt-button-float-bottom">
              ...
              onClick={() => this.handleSubmit("isApproved")}
              >
              Submit
              </Button>
            </Box>
