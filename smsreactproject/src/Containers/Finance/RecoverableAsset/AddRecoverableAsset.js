...
import { postRequest, getRequest, deleteRequest, patchRequest } from 'Includes/api/apicall';
...
// Mapping type options for grouping form
...
class AddRecoverableAsset extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            // SA payroll linking
            salaryAdvanceOptions: [],
            selectedSalaryAdvance: null,
            // Derived from category
            asset_type: null,
            isPendingFees: false,
            pendingFeesMode: 'manual',
            // Core fields
            name: '',
            opening_balance: '0',
    ...
    componentDidMount() {
        ...
        // Read URL query params — supports pre-selection from balance sheet lock prompt
        const params = new URLSearchParams(this.props.location?.search || '')
        const linkedModuleParam = params.get('linked_module')
        const modeParam = params.get('mode')
        const assetIdParam = params.get('asset_id')
        const normalIdParam = params.get('id')
        if (linkedModuleParam) {
            this._pendingLinkedModule = linkedModuleParam
        }
        if (modeParam === 'auto' || modeParam === 'manual') {
            this._pendingMode = modeParam
        }
        if (assetIdParam) {
            this._pendingAssetId = assetIdParam
        }
        if (normalIdParam) {
            this._normalEditId = normalIdParam
        }
    }
            ...
            .then(res => {
                if (res && res.status === 200) {
                    this.setState({
                        ...
                        if (this._normalEditId) {
                            this.loadNormalAssetForEdit(this._normalEditId)
                            delete this._normalEditId
                            return
                        }
                        // If editing an existing asset's mapping, load it first
                        if (this._pendingAssetId && this._pendingMode === 'auto') {
                            this.loadExistingAssetForMapping(this._pendingAssetId)
                            delete this._pendingAssetId
                            delete this._pendingLinkedModule
                            return
                        }
                        // If mode=auto and no asset_id, enter multi-asset mapping mode
                        if (this._pendingMode === 'auto' && this._pendingLinkedModule === 'SUNDRY_DEBTORS' && !this._pendingAssetId) {
                            delete this._pendingLinkedModule
                            this.loadPendingFeesAssets()
                            return
                        }
                        // If a linked module was requested via URL param, auto-select it
                        if (this._pendingLinkedModule) {
                            const mod = this._pendingLinkedModule
                            delete this._pendingLinkedModule
                            const option = LINKED_MODULE_OPTIONS.find(o => o.id === mod)
                            if (option) {
                                this.handleLinkedModuleChange(null, option)
                            }
                        }
                    })
                } else {
    ...
    // ── Load normal asset for standard editing ──
    loadNormalAssetForEdit = (assetId) => {
        getRequest(GET_URL.recoverableAsset.api + assetId + '/', {}, this.props)
            .then(res => {
                if (res && res.status === 200) {
                    const asset = res.data?.data || res.data
                    ...
                    let showStaff = false, showBank = false, showCounterparty = false, showRecovery = false;
                    if (matchingCategory) {
                        const assetTypes = matchingCategory.asset_types || []
                        const autoSections = []
                        ...
                        if (showStaff) this.loadStaffList()
                        if (showBank) this.loadBankList()
                    }
                    ...
                    this.setState({
                        ...
                        editAssetId: assetId,
                        selectedCategory: matchingCategory || null,
                        showStaff, showBank, showCounterparty, showRecovery,
                        name: asset.name || '',
                        asset_type: asset.asset_type || null,
                        ...
                        opening_balance: String(asset.opening_balance || 0),
                        opening_balance_type: asset.opening_balance_type || 'DEBIT',
                        staff: asset.staff_id ? { id: asset.staff_id } : (asset.staff ? { id: asset.staff } : null),
                        bank: asset.bank_id ? { id: asset.bank_id } : (asset.bank ? { id: asset.bank } : null),
                        linkedBankObject: asset.bank_id ? { id: asset.bank_id, opening_balance: asset.opening_balance, opening_balance_type: asset.opening_balance_type } : null,
                        account_label: asset.account_label ? { id: asset.account_label, name: asset.account_label } : null,
                        counterparty_name: asset.counterparty_name || '',
                        counterparty_type: asset.counterparty_type ? { id: asset.counterparty_type, name: asset.counterparty_type } : null,
                        total_amount: String(asset.total_amount || 0),
                        monthly_recovery_amount: String(asset.monthly_recovery_amount || 0),
                    })
                }
            })
    }
    ...
    loadSalaryAdvanceList = () => {
        if (this.state.salaryAdvanceOptions && this.state.salaryAdvanceOptions.length > 0) return
        getRequest(GET_URL.salaryAdvance.api, { is_active: true, limit: 500, pageno: 1 }, this.props)
            .then(res => {
                if (res && res.status === 200) {
                    var list = res.data.data && res.data.data.data_list ? res.data.data.data_list : (res.data.data || [])
                    this.setState({
                        salaryAdvanceOptions: list.map(sa => ({
                            id: sa.id,
                            name: sa.name + (sa.staff_name ? ` (${sa.staff_name})` : ''),
                            staff_name: sa.staff_name || '',
                            total_amount: sa.total_amount,
                            opening_balance: sa.opening_balance,
                            opening_balance_type: sa.opening_balance_type || 'DEBIT',
                            closing_balance: sa.closing_balance,
                        }))
                    })
                }
            })
        ...
        getRequest(GET_URL.bankdetail.api, { is_active: true }, this.props)
            .then(res => {
                if (res && res.status === 200) {
                    ...
                    this.setState({
                        bankOptions: bankData.map(b => ({
                            id: b.id,
                            name: b.display_name || b.bank_name,
                            opening_balance: b.opening_balance || 0,
                            opening_balance_type: b.opening_balance_type || 'DEBIT',
                        }))
                    })
                }
            })
    ...
    handleSubmit = () => {
        ...
        const isBankAccount = this.state.linkedModule === 'BANK_ACCOUNT'
        ...
        } else if (isBankAccount && this.state.linkedBankObject) {
            payload.opening_balance = parseFloat(this.state.linkedBankObject.opening_balance) || 0
            payload.opening_balance_type = this.state.linkedBankObject.opening_balance_type || 'DEBIT'
            payload.closing_balance = parseFloat(this.state.linkedBankObject.opening_balance) || 0
        ...
        if (this.state.linkedModule === 'STAFF_SALARY_ADVANCE') {
            ...
            if (this.state.selectedSalaryAdvance) {
                payload.salary_advance = this.state.selectedSalaryAdvance.id
            }
        }

        var url = POST_URL.recoverableAsset.api
        var requestMethod = postRequest
        if (this.state.isNormalEditMode && this.state.editAssetId) {
            url = GET_URL.recoverableAsset.api + this.state.editAssetId + '/'
            requestMethod = patchRequest
        }

        requestMethod(url, payload, this.props)
            .then(response => {
                if (response && response.status === 200 && !response.data?.error) {
                    ...
                    const msg = response?.data?.message || response?.data?.detail || 'Failed to save asset.';
                    Swal.fire('Error', msg, 'error');
                }
                this.setState({ submitDisable: false })
            })
            .catch((err) => {
                const msg = err?.response?.data?.message || err?.response?.data?.detail || 'Failed to save asset.';
                Swal.fire('Error', msg, 'error');
                this.setState({ submitDisable: false })
            })
    }
    ...
    renderAssetForm = () => {
        ...
        return (
            <Box>
                    ...
                    <Box mt={2}>
                        ...
                        <Box display="flex" alignItems="center" mb={2}>
                            <ToggleButtonGroup
                                ...
                                exclusive
                                disabled={state.isNormalEditMode}
                                onChange={(e, val) => {
                                    if (val) {
                ...
                {/* Staff Salary Advance — Link to existing SA */}
                {isSalaryAdvance && (
                    <Box>
                        {this.renderSectionHeader('Link to Salary Advance')}
                        <Typography variant="caption" style={{ color: '#64748b', display: 'block', marginBottom: 8 }}>
                            Select an existing salary advance record to link to this asset.
                        </Typography>
                        <Grid container spacing={3}>
                            <Grid item md={6} xs={12}>
                                <DropDownWithSearch
                                    options={state.salaryAdvanceOptions || []}
                                    value={state.selectedSalaryAdvance}
                                    onChange={(e, v) => {
                                        this.handleChange('selectedSalaryAdvance', v)
                                        if (v) {
                                            this.setState({
                                                name: v.name || '',
                                                opening_balance: v.opening_balance || '0',
                                                opening_balance_type: v.opening_balance_type || 'DEBIT',
                                            })
                                        }
                                    }}
                                    label="Salary Advance *" disabled={state.isNormalEditMode}
                                    fullWidth
                                    onOpen={this.loadSalaryAdvanceList}
                                />
                            </Grid>
                            {state.selectedSalaryAdvance && (
                                <Grid item md={6} xs={12}>
                                    <Box display="flex" alignItems="center" style={{ height: '100%' }}>
                                        <Typography variant="body2" style={{ color: '#1565c0' }}>
                                            Staff: {state.selectedSalaryAdvance.staff_name || '–'} |
                                            Principal: ₹{parseFloat(state.selectedSalaryAdvance.total_amount || 0).toLocaleString('en-IN')} |
                                            Outstanding: ₹{parseFloat(state.selectedSalaryAdvance.closing_balance || 0).toLocaleString('en-IN')}
                                        </Typography>
                                    </Box>
                                </Grid>
                            )}
                        ...
                        <Grid container spacing={3}>
                            <Grid item md={6} xs={12}>
                                <DropDownWithSearch
                                    ...
                                    label="Staff (Cash in Hand)" disabled={state.isNormalEditMode}
                                    fullWidth
                        ...
                        <Grid container spacing={3}>
                            <Grid item md={6} xs={12}>
                                <DropDownWithSearch
                                    ...
                                    value={state.bank}
                                    onChange={(e, v) => {
                                        this.handleChange('bank', v)
                                        this.setState({ linkedBankObject: v })
                                    }}
                                    label="Bank Account" disabled={state.isNormalEditMode}
                                    fullWidth
                                    onOpen={this.loadBankList}
                                />
                            </Grid>
                            {state.linkedBankObject && (
                                <Grid item md={6} xs={12}>
                                    <Box display="flex" alignItems="center" style={{ height: '100%' }}>
                                        <Typography variant="body2" style={{ color: '#1565c0' }}>
                                            Opening Balance: ₹{parseFloat(state.linkedBankObject.opening_balance || 0).toLocaleString('en-IN')} ({state.linkedBankObject.opening_balance_type || 'DEBIT'})
                                        </Typography>
                                    </Box>
                                </Grid>
                            )}
                            <Grid item md={6} xs={12}>
                                <DropDownWithSearch
                                    ...
                                    label="Account Label" disabled={state.isNormalEditMode}
                                    fullWidth
                                />
                            </Grid>
                        </Grid>
                                ...
                                <Grid container spacing={3}>
                                    <Grid item md={6} xs={12}>
                                        <DropDownWithSearch
                                            ...
                                            label="Staff Member" disabled={state.isNormalEditMode}
                                            fullWidth
                        ...
                        {showCounterparty && (
                            <Box>
                                ...
                                <Grid container spacing={3}>
                                    <Grid item md={6} xs={12}>
                                        <TextField
                                            fullWidth variant="outlined" label="Counterparty Name" disabled={state.isNormalEditMode}
                                    ...
                                    <Grid item md={6} xs={12}>
                                        <DropDownWithSearch
                                            ...
                                            label="Counterparty Type" disabled={state.isNormalEditMode}
                                            fullWidth
                                        />
                                    </Grid>
                                </Grid>
                        ...
                        {showRecovery && (
                            <Box>
                                ...
                                <Grid container spacing={3}>
                                    <Grid item md={6} xs={12}>
                                        <TextField
                                            fullWidth variant="outlined" label="Total Amount" disabled={state.isNormalEditMode}
                                    ...
                                    <Grid item md={6} xs={12}>
                                        <TextField
                                            fullWidth variant="outlined" label="Monthly Recovery Amount" disabled={state.isNormalEditMode}
                ...
                <Box display="flex" justifyContent="flex-end" mt={3}>
                    <Button
                        ...
                        style={{ minWidth: 120, padding: '10px 30px' }}
                    >
                        {state.submitDisable ? (this.state.isNormalEditMode ? 'Updating...' : 'Saving...') : (this.state.isNormalEditMode ? 'Update' : 'Submit')}
                    </Button>
                </Box>
            </Box>
        )
        ...
        return (
            <Paper className={classNames('paper-background')}>
                ...
                <Grid container>
                    <Grid item md={8} xs={12} className={classNames('header-align')}>
                        <Box className="heading">
                            {this.state.isNormalEditMode ? 'Edit Recoverable Asset' : 'Add Recoverable Asset'}
                        </Box>
                        <Box className="sub-heading">
                            {selectedCategory
                            ...
                            <Grid container spacing={3}>
                                <Grid item md={6} xs={12}>
                                    <DropDownWithSearch
                                        ...
                                        label="Category *" disabled={this.state.isNormalEditMode}
                                        className="width-100"
                                    />
                                    {errors.category && (
                                ...
                                <Grid item md={6} xs={12}>
                                    <DropDownWithSearch
                                        ...
                                        label="Link to Module" disabled={this.state.isNormalEditMode}
                                        className="width-100"
                                    />
                                </Grid>
                            </Grid>
...
export default withRouter(AddRecoverableAsset)
