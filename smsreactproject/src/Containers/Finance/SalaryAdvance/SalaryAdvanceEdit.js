...
import { Grid, Paper, Box, Button, Typography, Divider, CircularProgress } from '@material-ui/core';
...
import { dateFormat, isUserHasPermission, isObjectValuesEmpty, numberWithCommas, getUrlParam } from 'Includes/functions';


class SalaryAdvanceEdit extends Component {
            ...
            fieldDetails: [
                {
                    ...
                    list: [], customName: 'name', disabled: false // Will be disabled if transactions exist
                },
                {
                ...
                {
                    label: 'Asset Name', regex: nameWithQuoteRegex, name: 'asset_name', md: 6, maxLength: 255, className: 'md-up-width-85', required: true,
                    id: 'outlined-textarea-asset-name', default: '', rows: null, type: 'text',
                    categoryField: true
                },
                {
    ...
    loadAdvanceData = async () => {
        const { advanceId, fieldDetails } = this.state

        try {
            const advanceResponse = await getRequest(`${GET_URL.salaryAdvance.api}${advanceId}/`, {}, this.props)
            if (!advanceResponse || advanceResponse.status !== 200) {
                Swal.fire('Error', 'Failed to load salary advance data', 'error')
                this.props.history.push(this.viewUrl)
                return
            }

            const advanceData = advanceResponse.data.data || advanceResponse.data

            const txnResponse = await getRequest(GET_URL.salaryAdvanceTransaction.api, { salary_advance: advanceId, limit: 1 }, this.props)
            const hasTransactions = (txnResponse?.data?.data?.data_list?.length || 0) > 0

            const staffResponse = await getRequest(GET_URL.staff.api, { is_active: true, limit: 100 }, this.props)
            if (staffResponse && staffResponse.status === 200) {
                let staffList = staffResponse.data.data.data_list || staffResponse.data.data || []

                if (advanceData.staff && !staffList.find(s => s.id === advanceData.staff)) {
                    staffList.unshift({
                        id: advanceData.staff,
                        name: advanceData.staff_name || 'Current Staff',
                        first_name: advanceData.staff_name || 'Current',
                        last_name: ''
                    })
                }
                ...
                fieldDetails.forEach(field => {
                    if (field.name === 'staff') {
                        ...
                        field.disabled = hasTransactions
                    }
                    if (field.name === 'approved_by') {
                        ...
                        if (advanceData.approved_by && !field.list.find(s => s.id === advanceData.approved_by)) {
                            field.list.unshift({
                                id: advanceData.approved_by,
                                name: advanceData.approved_by_name || 'Approver'
                            })
                        }
                    }
                })
            ...
            try {
                const catParams = { is_active: true, limit: 100 }
                const catResponse = await getRequest(GET_URL.recoverableAssetCategory.api, catParams, this.props)
            ...
            const advanceType = advanceData.advance_type || 'SALARY_ADVANCE'
            const payloadData = {
                staff: advanceData.staff || '',
                financial_year: advanceData.financial_year || '',
                category: advanceData.category || '',
                advance_type: advanceType,
                description: advanceData.purpose || advanceData.name || '',
                total_amount: advanceData.total_amount || advanceData.opening_balance || '',
                monthly_recovery_amount: advanceData.monthly_recovery_amount || '',
                tenure_months: advanceData.tenure_months || '',
                start_month: advanceData.start_month ? new Date(advanceData.start_month) : null,
                approved_on: advanceData.approved_on ? new Date(advanceData.approved_on) : null,
                approved_by: advanceData.approved_by || '',
                interest_type: advanceData.interest_type || 'NONE',
                interest_rate: advanceData.interest_rate || 0,
                penalty_rate: advanceData.penalty_rate || 0,
                auto_deduct_from_payroll: advanceData.auto_deduct_from_payroll ? 'YES' : 'NO',
                deduction_priority: advanceData.deduction_priority || 1,
                asset_name: advanceData.name || '',
            ...
            this.setState({
                fieldDetails,
                originalData: advanceData,
                payloadData,
                hasTransactions,
                fieldValues: this.getFieldsForType(advanceType, fieldDetails, payloadData),
                loading: false
            })
        } catch (e) {
    ...
    submit = () => {
        ...
        if (this.validate(payload)) {
            const formattedPayload = {
                ...
                name: payload.asset_name || payload.description || `Staff ${payload.advance_type || 'SALARY_ADVANCE'}`,
            ...
            const url = `${PUT_URL.salaryAdvance.api}${this.state.advanceId}/`
                ...
                .then((response) => {
                    if (response && response.status === 200) {
                        Swal.fire({
                            position: 'top-end',
                            type: 'success',
                            title: 'Salary Advance has been updated',
    ...
    render() {
        const { submitDisable, loading, fieldValues, payloadData, calculatedEmi, hasTransactions } = this.state
                    ...
                    <Grid container>
                        <Grid item md={6} xs={12} className={classNames('header-align')}>
                            <Box className='heading'>
                                Edit {isLoan ? 'Staff Loan' : 'Salary Advance'}
                            </Box>
                            <Box className='sub-heading'>
                                Modify the {isLoan ? 'loan' : 'salary advance'} details
                                {hasTransactions && (
                                    <Typography variant="caption" color="secondary" style={{ display: 'block', marginTop: 5 }}>
                                        Note: Staff cannot be changed as transactions exist
                                    </Typography>
                                )}
                            </Box>
                        </Grid>
...
export default withRouter(SalaryAdvanceEdit)
