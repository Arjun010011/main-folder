...
class EvaluateStudentHomeWork extends Component {
  ...
  acceptStudentDiary = () => {
    const { points, acceptDescription,remarks } = this.state;
    const teacherReviewData = {
      status: "Completed",
      marks: points ? points : null,
    };
    this.uploadRemarks({ comment: acceptDescription, remarks: remarks }, teacherReviewData);
    ...
    if (status === "accept") {
      ...
      Swal.fire({
        ...
        if (result.value) {
          ...
          this.uploadRemarks({remarks: ""}, teacherReviewData);
        }
      });
    }
  };
