...
import CreateQuestionSurveyForm from 'Containers/SurveyForm/components/CreateQuestionSurveyForm'
import CreateQuestionVideoSurveyForm from 'Containers/SurveyForm/components/CreateQuestionVideoSurveyForm'
import { getRequest } from 'Includes/api/apicall';
import { GET_URL } from 'Includes/urls';
import { Actions } from 'Constants/permissions';
import loadingBar from 'images/loading.gif';
import ResponsesList from 'Containers/SurveyForm/ResponsesListSurveyForm';
import { getUrlParam } from 'Includes/functions';
import LoadingGif from 'Components/LoadingGif';

import './styles.scss';
import SummarySurveyForm from './SummarySurveyForm';

class SetSurveyForm extends Component {
    constructor(props) {
        super(props);
        this.state = {
            currentTab: 'quizForm',
    ...
    componentDidMount() {
        ...
        if (year_name && year && start_date && end_date ) {
            if (this.props.location.pathname === Actions.set_surveyform.update.url) {
                if (this.props.location.state && this.props.location.state.detail) {
                    this.getQuizDetails(this.props.location.state.detail);
                }
                else {
                    this.props.history.push(Actions.set_surveyform.view.url);
                }
            }
            else {
                this.setState({
    ...
    getQuizDetails = (id) => {
        const url = GET_URL.feedbackforms.api + id + '/'
    ...
    goToViewPage = () => {
        ...
        this.props.history.push({
            pathname: Actions.set_surveyform.view.url,
            search: searchParam,
        });
    }
                        ...
                        <Grid container>
                            ...
                            <Grid item lg={2} md={2} xs={12} >
                                <Box className={'end-flex-prop'}>
                                    ...
                                    ><VisibilityOutlinedIcon className='visibility-icon' /> {Actions.set_surveyform.view.label}</Button>
                                </Box>
                            </Grid>
                        </Grid>
...
export default withRouter(SetSurveyForm)