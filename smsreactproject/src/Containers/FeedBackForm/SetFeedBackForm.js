...
import CreateQuestionFeedBackForm from 'Containers/FeedBackForm/components/CreateQuestionFeedBackForm'
import CreateQuestionVideoFeedBackForm from 'Containers/FeedBackForm/components/CreateQuestionVideoFeedBackForm'
import { getRequest } from 'Includes/api/apicall';
import { GET_URL } from 'Includes/urls';
import { Actions } from 'Constants/permissions';
import loadingBar from 'images/loading.gif';
import ResponsesList from 'Containers/FeedBackForm/ResponsesListFeedBackForm';
...
class SetFeedBackForm extends Component {
    constructor(props) {
        super(props);
        this.state = {
            currentTab: 'quizForm',
    ...
    componentDidMount() {
        ...
        if (year_name && year && start_date && end_date ) {
            if (this.props.location.pathname === Actions.set_feedbackform.update.url) {
                if (this.props.location.state && this.props.location.state.detail) {
                    this.getQuizDetails(this.props.location.state.detail);
                }
                else {
                    this.props.history.push(Actions.set_feedbackform.view.url);
                }
            }
            else {
                this.setState({
                        ...
                        <Grid container>
                            ...
                            <Grid item lg={2} md={2} xs={12} >
                                <Box className={'end-flex-prop'}>
                                    ...
                                    ><VisibilityOutlinedIcon className='visibility-icon' /> {Actions.set_feedbackform.view.label}</Button>
                                </Box>
                            </Grid>
                        </Grid>
                            ...
                            <Box className='leave-height-100-percent'>
                                ...
                                }
                                {
                                    ...
                                    ((is_video === 'true' && !is_finalized)?
                                        <CreateQuestionVideoFeedBackForm
                                            current_standard={current_standard}
                                            year={year}
                                            end_date={end_date}
                                            isEdit={isEdit}
...
export default withRouter(SetFeedBackForm)