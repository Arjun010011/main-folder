...
class ManageProgramOutcome extends Component {
    constructor() {
        super();
        this.state = {
            subjects: [],
    ...
    getProgramOutcomeList = () => {
        let { programoutcomeDetails } = this.state;
        programoutcomeDetails = programoutcome_global;
        this.setState({
            programoutcomeDetails,
            loading: false,
        });
    ...
    updateProgramOutcomeValue = (stateValue) => {
        let { programoutcome } = this.state;
        programoutcome = stateValue;
        this.setState({
            programoutcome,
        });
    };
    ...
    validate = () => {
        let stateTest = true;
        let branchTest = true;
        let { programoutcome, fieldError, branch_list } = this.state;
        stateTest = this.refs.state.validateFields();
        console.log(programoutcome)

        let post_data = programoutcome;
        this.setState({ submitDisable: true });
        let url = POST_URL.programoutcome.api;
        postRequest(url, post_data, this.props).then((response) => {
            if (response && response.status === 200) {
                ...
                this.props.history.push(Actions.programoutcomedata.view.url);
            }
                        ...
                        <Grid container>
                            ...
                            <Grid item md={6} xs={12}>
                                <Box className={classNames("header-align", "end-flex-prop")}>
                                    {isUserHasPermission("programoutcome", "view") && (
                                        <Button
                                            variant="contained"
                                        ...
                                        >
                                            ...
                                            {Actions.programoutcomedata.view.label}
                                        </Button>
                                    )}
                                </Box>
                            </Grid>
                        ...
                        <Grid container className={classNames("header-align")}>
                            <Grid item md={10} xs={12}>
                                <MultipleAddTextFields
                                    fieldDefaultValue={[]}
                                    fieldDetails={programoutcomeDetails}
                                    updateParent={this.updateProgramOutcomeValue}
...
export default withRouter(ManageProgramOutcome);