...
const stateDetails_global = [
    {
        label: 'District Name', regex: nameRegex, autoFocus: true, name: 'name', md: 6, className: 'width-form-95',
...
class ManageDistricts extends Component {
    ...
    componentDidMount = () => {
        if (this.props.location.state) {
            ...
            this.props.history.push(Actions.manage_districts.view.url)
        }
    }


    updatedistrictListValue = (stateValue) => {
        let { districtList } = this.state
        districtList = stateValue
        this.setState({
            districtList
        })
    }
    ...
    validate = () => {
        let districtTest = true;
        let { districtList, selectedCountry, error, selectedState } = this.state
        districtTest = this.refs.district.validateFields();
        ...
        if (districtTest && selectedCountry) {
            let post_data = {
                ...
                .then((response) => {
                    if (response && response.status === 200) {
                        ...
                        this.props.history.push({
                            pathname: Actions.manage_districts.view.url,
                            state: { selectedCountry: selectedCountry, selectedState: selectedState }
                        })
                    }
    ...
    render() {
        const { loading, countryName, stateName, open } = this.state
        if (loading) {
            return (
                <Box display='flex'>
                        ...
                        <Grid container>
                            ...
                            <Grid item md={6} xs={12} >
                                <Box className={classNames('header-align', 'end-flex-prop')}>
                                    {isUserHasPermission('manage_districts', 'view') && <Button
                                        variant="contained"
                                        onClick={this.handleViewButton}
                                        className='editbutton-view'
                                    ><VisibilityOutlinedIcon className='visibility-icon' /> {Actions.manage_districts.view.label}</Button>}
                                </Box>
                            </Grid>
                        </Grid>
                        ...
                        <Grid container className={classNames('header-align')}>
                            <Grid item md={6}>
                                <MultipleAddTextFields
                                    ...
                                    updateParent={this.updatedistrictListValue}
                                    isEmptyNotAllowed={true}
                                    ref={'district'}
                                    idFormat={'district_add_2022_08_11_2_pm_'}
                                />
...
export default withRouter(ManageDistricts)
