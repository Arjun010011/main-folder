...
const fieldDetails = [
    {
        label: 'District Name', regex: nameRegex, name: 'name', md: 12, className: 'width-100', required: true,
...
class DistrictView extends Component {
                        ...
                        customBodyRender: (value, tableMeta, updateValue) => {
                            return (<div>
                                <ActionColumn
                                    ...
                                    label='Please Update District Details'
                                    fieldDetails={fieldDetails}
                                    updateUrl={PUT_URL.district.api}
                                    updatePostFormat={this.updatePostFormat}
                                    updateType={this.updateType}
                                    deleteUrl={DEL_URL.district.api}
    ...
    updatePostFormat = (newData) => {
        let { selectedCountry, selectedState } = this.state
        let payload = {
            name: newData.name,
    ...
    updatePermissions = (name) => {
        let test = true
        const hasEditPermission = isUserHasPermission('manage_districts', 'update')
        const hasDeletePermission = isUserHasPermission('manage_districts', 'delete')
        let permissions = [];
        let enabledActions = []
        if (hasEditPermission) {
            enabledActions.push('edit')
            permissions.push('manage_districts');
        }
    ...
    updateType = (newData, id) => {
        ...
        let district = this.state.districtList
        district.map((data, index) => {
            if (data.id === id) {
                district[index].name = newData.name
            }
        })
        this.setState({
    ...
    handleAddButton = () => {
        let { selectedCountry, error, alertData, countryList, selectedState, stateList } = this.state;
        if (selectedCountry && selectedCountry !== 0 && selectedState && selectedState !== 0) {
            ...
            this.props.history.push({
                pathname: Actions.manage_districts.create.url,
                state: {
            ...
            error.selectedCountry = alertData
            this.setState({
                open: true,
                alertData,
                error
            ...
            alertData = 'Please select State'
            error.selectedState = alertData
            this.setState({
                open: true,
                alertData,
                error
    ...
    render() {
        const { loading, countryList, selectedCountry, stateList, selectedState, districtList, columns, options, tableUpdating,
            stateLoading, open, error, alertData
        } = this.state
        if (loading) {
            return (
                <Box display='flex'>
                        ...
                        <Grid container>
                            <Grid item md={6} xs={12} className={classNames('header-align')}>
                                ...
                                <Box className='sub-heading'>
                                    {`The District schedule of the ${alias_names['school']} is defined here over a period time.The academic year
                                    over 12 months of time.`}
                                    </Box>
                            </Grid>
                            <Grid item md={6} xs={12} >
                                <Box className={classNames('header-align', 'end-flex-prop')}>
                                    {isUserHasPermission('manage_districts', 'create') && <Button
                                        variant="contained"
                                        onClick={this.handleAddButton}
                                        className='editbutton-view'
                                    ><AddCircleOutlineOutlinedIcon className='visibility-icon' /> {Actions.manage_districts.create.label}</Button>}
                                </Box>
                            </Grid>
                        </Grid>
