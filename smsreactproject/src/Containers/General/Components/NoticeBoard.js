...
import SelectStandardSection from "./SelectStandardSection";
...
const select_types = [
  ...
  { id: 'transport', name: 'Transport Route' },
];
...
class NotificationCreate extends Component {
      ...
      this.setState(
        ...
        () => {
          if (selected_type === "group") {
            ...
            this.setState({
              selected_route: selected_details.updated_list,
            });
            this.getRoutePlanName();
          }
        }
      );
    } else {
  ...
  getGroupList = () => {
    ...
    getRequest(GET_URL.groups.api, {}, this.props).then((response) => {
      if (response && response.status === 200) {
  ...
  handleSubmit = () => {
    ...
    if (selected_type === "group") {
      ...
      selected_details["updated_list"] = [...selected_route];
    }
            ...
            {selected_type === "transport" && (
              <Grid item md={3} xs={12}>
                <MultipleSelectDropdown
                  data_list={routeList}
                  selected_list={selected_route}
                  label={"Select Routes"}
                  onChange={(e) => this.onRouteChange(e, "transport")}
                />
              </Grid>
            )}
