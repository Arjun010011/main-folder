...
class EvaluateHomeWorkList extends Component {
    constructor() {
        super()
    ...
    goToEvaluatePage = (id) => {
        ...
        this.props.history.push({
            pathname: Actions.diary_managehomework_evaluate_student.view.url,
            search: searchParam,
        });
    }
    ...
    handleChangeDateRange = (value, isOpened) => {
        ...
        this.setState({
            ...
            this.getEvaluateHomeWorklist()
        })
    }

    getEvaluateHomeWorklist = (paginationProps) => {
        ...
        let params = { ...pagination_params, evaluate_data: 1 , status : 'NOT_COMPLETED'};
        ...
        getRequest(url, params).then((response) => {
            if (response && response.status === 200) {
                const evaluate_list = response.data.data;
                this.setState({
                    evaluate_list,
                    tableUpdating: false,
    ...
    onChangeDateRangeDropdown = (e) => {
        ...
        let { dateRangeValue, dateRangeDropdown } = this.state;
        let start, end;
        if (value === 'today') {
            start = dateFormat(new Date(), 'YYYY-MM-DD')
            end = dateFormat(new Date(), 'YYYY-MM-DD')
    ...
    render() {
        const { loading, evaluate_list, pagination, dateRangeDropdownList, dateRangeDropdown, dateRangeValue,
            tableUpdating, isEvaluatePage, homework } = this.state
        const options = {
            selectableRows: "none",
            filterType: "dropdown",
            responsive: "simple",
                ...
                <Box>
                    ...
                    <Grid container className={classNames('header-align')}>
                        <Grid item md={12} xs={12}>
                            <AllMUIDataTable
                                key={evaluate_list.data_list}
                                title={tableUpdating ? <CircularProgress className='white-text' /> :
                                    `${dateFormat(dateRangeValue.start, 'DD-MM-YYYY')} TO ${dateFormat(dateRangeValue.end, 'DD-MM-YYYY')}`}
                                data={evaluate_list.data_list}
                                columns={this.evaluate_columns}
                                options={options}
                                onTableChange={this.getEvaluateHomeWorklist}
                                serverSide={true}
                                pagination={pagination}
                                count={evaluate_list.count}
                            />
                        </Grid>
                    </Grid>
                </Box>
...
export default withRouter(EvaluateHomeWorkList)