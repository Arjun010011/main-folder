...
import { dateFormat, isUserHasPermission, timeFormat, getPreviousDate, getFinancialYear, SetFinancialYear } from 'Includes/functions';
...
class HrAssignShiftView extends Component {
    constructor() {
        super()
        this.state = {
            assignShiftList: [],
                        ...
                        customBodyRender: (value, tableMeta, updateValue) => {
                            return (<div>
                                <ActionColumn
                                    ...
                                    label={`Update Shift To ${tableMeta.rowData[1]}`}
    ...
    updatePermissions = (name) => {
        let test = true
        const hasEditPermission = isUserHasPermission('assign_shift', 'update')
        const hasDeletePermission = isUserHasPermission('assign_shift', 'delete')
        let permissions = [];
        let enabledActions = []
        if (hasEditPermission) {
            enabledActions.push('edit')
            permissions.push('assign_shift');
        }
    ...
    getAssignShiftList = (fromDate, toDate) => {
        ...
        const params = { is_active: true, fromdate: fromDate, todate: toDate }
    ...
    render() {
        ...
        const options = {
            ...
            downloadOptions: {
                filename: "Employee_Shift_Assigned.csv",
                filterOptions: {
            ...
            return (
                <Box>
                    <Paper className={classNames('paper-background')}>
                        <Grid container>
                            ...
                            <Grid item md={6} xs={12} >
                                <Box className={classNames('header-align', 'end-flex-prop')}>
                                    {isUserHasPermission('assign_shift', 'create') && <Button
                                        variant="contained"
                                        component={Link} to={Actions.assign_shift.create.url}
                                        className='editbutton-view'
                                    ><AddCircleOutlineOutlinedIcon className='visibility-icon' /> {Actions.assign_shift.create.label}</Button>}
                                </Box>
                            </Grid>
                        </Grid>
                        <Box className='header-align'>
                        ...
                        <Box className='staff-list-assigned-shift'>Note : Staffs added with both custom date range and financial year are listed below.</Box>
...
export default HrAssignShiftView
