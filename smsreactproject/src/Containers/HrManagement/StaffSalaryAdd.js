...
import { Paper, Box, Grid, Button, MenuItem, Checkbox, TextField, FormControl, FormHelperText, CircularProgress } from '@material-ui/core';
...
import { DEFAULT_PAGINATION_PROPS_FIRST_NAME_LIST, IMPORT_CONFIGURATION_STAFF_LIST } from 'Constants';
...
class StaffSalaryAdd extends Component {
    ...
    validation = (isSubmit) => {
        ...
        finalStudentList.map((parent, pIndex) => {
            if (parent.username && parent.checked) {
    ...
    handleViewButton = () => {
        this.props.history.push({
            pathname: Actions.staff_salary.view.url,
        })
    }
    ...
    render() {
        const { loading, showPassword, openError, alertData, searchStudent, finalStudentList,selectAll,
                        ...
                        <Grid container>
                            ...
                            <Grid item md={4} xs={12} >
                                <Box className='header-align end-flex-prop'>
                                    {isUserHasPermission('staff_salary', 'view') &&
                                        <Button
                                            variant="contained"
                                            onClick={this.handleViewButton}
                                            // component={Link} to={Actions.hostel_student_transaction_list.view.url}
                                            className='editbutton-view'
                                        ><VisibilityOutlinedIcon className='visibility-icon' /> {Actions.staff_salary.view.label}</Button>
                                    }
                                </Box>

                            </Grid>
                        </Grid>
                        <Paper className='plain-paper-background pl-20 pt-20  mt-20'>
                            <div>
                                ...
                                <table width="100%" className="selectable-row-table mt-20">
                                    <thead className='table-select-hostel-thead'>
                                        <th className={`selectable-table-head`}>
                                            <MenuItem value={selectAll} onClick={() => this.onChangeSelectAll()} className='padding-0' >
                                                <Checkbox color='secondary' checked={selectAll} className='padding-0'/>
                                            </MenuItem>
                                        </th>
                                        <th className={`selectable-table-head`}> Staff Name  </th>
                                        {/* <th className={`selectable-table-head`}> Admission No.  </th> */}
                                        <th className={`selectable-table-head`}> Salary </th>
                                    </thead>
                                        ...
                                        {finalStudentList.map((student, index) => {
                                            return (
                                                <tr key={index} className={student.is_low_balance ? "selectable-row-table-row text-red" : "selectable-row-table-row"}>
                                                    <td className='pl-15'>
                                                        <MenuItem value={student.checked} onClick={() => this.onChangeSelect(index)} className='padding-0' >
                                                            <Checkbox color='secondary' checked={student.checked} className='padding-0' />
                                                        </MenuItem>
                                                    </td>
...
export default withRouter(StaffSalaryAdd)