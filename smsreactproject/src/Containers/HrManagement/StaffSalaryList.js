...
import { isUserHasPermission, dateFormat, getIsGridOrListView } from 'Includes/functions';
...
class StaffSalaryList extends Component {
        ...
        options['onDownload'] = (buildHead, buildBody, columns, data) => {
            data.map((columnValue, index) => {
                columnValue.data.map((objectValue, subIndex) => {
                    if (subIndex == 5) {
                        data[index]['data'][subIndex] = dateFormat(objectValue, 'DD-MM-YYYY')
                    }
                })
            })
    ...
    render() {
        let { loading, staffList, tableUpdating, options } = this.state
        if (loading) {
            return <LoadingGif />
        }
        else {
            return (
                <Paper className={classNames('paper-background')}>
                    <Grid container>
                        <Grid item lg={7} md={4} xs={12} className={classNames('header-align')}>
                        ...
                        <Grid item lg={5} md={3} xs={12} >
                            <Box className={classNames('header-align', 'end-flex-prop')}>
                                {isUserHasPermission('staff_salary', 'create') && <Button
                                    variant="contained"
                                    component={Link} to={Actions.staff_salary.create.url}
                                    className='editbutton-view'
                                ><AddCircleOutlineIcon className='visibility-icon' /> {Actions.staff_salary.create.label}</Button>}
                            </Box>
                        </Grid>
                    </Grid>
