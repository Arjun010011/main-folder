...
class StudentBulkUpdateList extends Component {
  constructor() {
    super();
  ...
  getStudentList = (paginationProps) => {
    let { pagination, year, current_standard, dateRangeValue, student_type } =
      this.state;
  ...
  handleAddAdmissionButton = () => {
    let { year, error, alertData, yearList, standardList, current_standard } =
      this.state;
    if (!year) {
    ...
    this.props.history.push({
      pathname: Actions.student_bulk_update.update.url,
      search: searchParam,
    });
  };
        ...
        <Paper className={classNames("paper-background")}>
          <Grid container>
            <Grid item md={6} xs={12} className={classNames("header-align")}>
              <Box className="heading">Student Bulk List</Box>
            </Grid>
            <Grid item md={6} xs={12}>
              <Box className={classNames("header-align", "end-flex-prop")}>
                {isUserHasPermission("student_bulk_update", "update") && (
                  <Button
                    variant="contained"
...
export default withRouter(StudentBulkUpdateList);
