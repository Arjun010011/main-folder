...
import FeePaymentModeModify from "Containers/Finance/FeeCollection/FeePaymentModeModify";
import {
...
function StudentFeeCollection(props) {
  ...
  const [isPaymentModeDialog, setIsPaymentModeDialog] = React.useState(false);
  const [paymentModeInfo, setPaymentModeInfo] = React.useState({});
  ...
  const pushToFeesCollection = (yearId, standardId, id) => {
    let searchState = {
      yearid: yearId,
    ...
    const columns = [
      ...
      },
      {
        ...
        options: {
          ...
          sort: false,
          download: false,
          display: isUserHasPermission("fee_collection", "create"),
          customBodyRender: (value, tableMeta) => {
            ...
            return (
              <>
                {!is_rte_fee_enabled && tableMeta.rowData[11] === "RTE" && !tableMeta.rowData[10] ? (
                  <>
                    <Button
                      size="small"
                  ...
                  >
                    ...
                    {tableMeta.rowData[4] && tableMeta.rowData[3] > 0 && (
                      <Button
                        size="small"
  ...
  const handleCancel = (data, index, yearId) => {
  ...
  const handleEditPaymentMode = (data, index, yearId) => {
    let paymentModeInfoTemp = {
      paymentError: "",
      transId: data["id"],
      totalAmount: data["total_amount"],
      existingPayments: data["mode_of_payment_list"] || [],
      yearId: yearId,
      editIndex: index,
    };
    setPaymentModeInfo(() => paymentModeInfoTemp);
    setIsPaymentModeDialog(() => true);
  };

  const updatePaymentMode = (payments) => {
    const url =
      PUT_URL.feecollection.api + paymentModeInfo["transId"] + "/";
    let post_data = {
    ...
    putRequest(url, post_data, prop, true).then((response) => {
      if (response && response.status === 200) {
        let paymentDetailTemp = { ...paymentDetail };
        // Update mode_of_payment_list in local state
        paymentDetailTemp[
          `${paymentModeInfo["yearId"]}${props.student_id}`
        ][paymentModeInfo.editIndex]["mode_of_payment_list"] = payments.map(p => ({
        ...
        Swal.fire({
          position: "top-end",
          type: "success",
          title: response.data.Reason || "Payment mode updated successfully",
          showConfirmButton: false,
          timer: 1500,
        });
        setIsPaymentModeDialog(() => false);
        setPaymentModeInfo(() => ({}));
      } else {
        let paymentModeInfoTemp = { ...paymentModeInfo };
        paymentModeInfoTemp["paymentError"] = response || "Failed to update payment mode";
        setPaymentModeInfo(() => paymentModeInfoTemp);
      }
    });
  };
  ...
  return (
    <div className="mt-30">
      ...
      {cancelReasonOpen && (
        <FeeCancelReason
          handleReasonClose={() => setCancelReasonOpen(() => false)}
          deleteReciept={deleteReciept}
          delError={deleteInformation.delError}
        />
      )}

      {isTransactionDialog && (
        <FeeTransactionModify
          transactionDate={transactionInformation["transactionDate"]}
          updateTransaction={updateTransaction}
          handleTransactionClose={() => setIsTransactionDialog(() => false)}
          transError={transactionInformation["transError"]}
        />
      )}

      {isPaymentModeDialog && (
        <FeePaymentModeModify
          totalAmount={paymentModeInfo["totalAmount"]}
          existingPayments={paymentModeInfo["existingPayments"]}
          transactionId={paymentModeInfo["transId"]}
          updatePaymentMode={updatePaymentMode}
          handlePaymentModeClose={() => setIsPaymentModeDialog(() => false)}
          paymentError={paymentModeInfo["paymentError"]}
        />
      )}
    </div>
  );
