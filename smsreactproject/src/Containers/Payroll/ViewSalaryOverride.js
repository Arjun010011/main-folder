...
class ViewSalaryOverride extends Component {
    constructor(props) {
        super(props);
                ...
                },
                {
                    ...
                    options: {
                        ...
                        customBodyRender: (value, tableMeta) => {
                            let isPaid = tableMeta.rowData[7];
                            let displayValue = isPaid
                                ? <FormattedMessage {...messages.viewOverrides} />
                                : <FormattedMessage {...messages.configureOverride} />;
                            return (
                                <div className='action-cloumn-width'>
                                    <Button
                                        className='add-modify-button'
                                        onClick={() => this.configureOverride(
                                            tableMeta.rowData[6],
                                            isPaid
                                        )}
                                    >
    ...
    componentDidMount() {
        ...
        if (pagination_types['salary_override']) {
            pagination_temp['page'] = pagination_types['salary_override']['page']
            pagination_temp['rowsPerPage'] = pagination_types['salary_override']['rowsPerPage']
        }
        this.setState({
    ...
    configureOverride = (id, salaryIsPaid) => {
        let { year, yearName, salaryMonth, salaryMonthName, month } = this.state;
        this.props.history.push({
            pathname: Actions.payroll_salaryoverride.create.url,
            state: {
    ...
    onTableChange = (tableState) => {
        ...
        const pagination_types = JSON.parse(localStorage.getItem('pagination_types')) ? JSON.parse(localStorage.getItem('pagination_types')) : {}
        let temp = { salary_override: newOptions }
    ...
    render() {
        let { loading, staffList, columns, optionsLocal, tableLoading, month, year, yearList, monthList } = this.state;
                    ...
                    <Grid container>
                        <Grid item md={6} xs={12} className={classNames('header-align')}>
                            <Box className='heading'>
                                <FormattedMessage {...messages.salaryOverride} />
                            </Box>
                        </Grid>
                    </Grid>
...
export default withRouter(ViewSalaryOverride)
