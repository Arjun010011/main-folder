...
import StudentEnableFeatureTable from "./Components/StudentMemberShipList";
...
class MembershipLibrary extends Component {
  state = {
    loading: true,
    yearList: [],
    standardList: [],
  ...
  getAcademicYear = () => {
    const params = {};
    getRequest(GET_URL.getacademicyear.api, params, {}).then((response) => {
  ...
  getStudentList = (paginationProps) => {
    ...
    let params = {
      ...
      library_membership_data: true,
    };
    if (standard && standard != "all") {
      params["standard"] = standard;
    }
    getRequest(GET_URL.student.api, params, {
      return_error_message: true,
    }).then((response) => {
      if (response && response.status === 200) {
        const studentList = response.data;
        studentList.data.student_list.map((data) => {
          data["standard"] = data["current_standard_section_name"]
  ...
  getBlankPageMessage = (message = "") => {
    ...
    if (message === "") {
      ...
      if (standard === 0) {
        message = `Select ${alias_names["standard"]} to view the student list`;
      }
    }
    this.setState({
      blankPageMessage: message,
      loading: false,
      pageloading: false,
    });
  };
        ...
        <Box className="paper-background">
          <Grid container>
            <Grid item md={12} xs={12} sm={12}>
              <Box className="header-align heading" mt={1}>
                Membership
              </Box>
              <Box>
...
export default MembershipLibrary;
