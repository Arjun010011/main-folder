...
class ListRacks extends Component {
  constructor() {
    super();
    this.permission = updatePermissions("library_racks", [
      "update",
      "delete",
    ]);
    this.state = {
              ...
              return (
                <div>
                  <ActionColumn
                    ...
                    label={<FormattedMessage {...messages.editLibRacks} />}
                    fieldDetails={fieldDetails}
                    baseClassName="action-basic-detail-width"
                    updateUrl={PUT_URL.libraryrack.api}
                    updatePostFormat={this.updatePostFormat}
                    updateType={this.updateType}
                    deleteUrl={DEL_URL.libraryrack.api}
  ...
  getCategoryTypeList = () => {
    ...
    let url = GET_URL.libraryrack.api;
  ...
  updatePostFormat = (newData) => {
    let payload = {
      name: newData.name, level: newData.level
    };
    return payload;
  };
            ...
            <Grid container>
              <Grid item md={7} xs={12} sm={12}>
                <div className="header-align heading">
                  <FormattedMessage {...messages.libRacks} />
                </div>
              </Grid>
              <Grid item md={5} xs={12} sm={12}>
                <div className="end-flex-prop header-align">
                  {isUserHasPermission("library_racks", "create") && (
                    <Button
                      variant="contained"
                      component={Link}
                      to={Actions.library_racks.create.url}
                      className="editbutton-view"
                    >
                      <AddCircleOutlineOutlinedIcon className="visibility-icon" />{" "}
                      {Actions.library_racks.create.label}
                    </Button>
                  )}
                </div>
              </Grid>
