...
class LibCategoryList extends Component {
  constructor() {
    super();
    this.permission = updatePermissions("library_categegory", [
      "update",
      "delete",
    ]);
    this.state = {
              ...
              return (
                <div>
                  <ActionColumn
                    ...
                    label={
                      <FormattedMessage {...messages.editlibCategoryType} />
                    }
                    fieldDetails={fieldDetails}
                    baseClassName="action-basic-detail-width"
                    updateUrl={PUT_URL.librarycategory.api}
                    updatePostFormat={this.updatePostFormat}
                    updateType={this.updateType}
                    deleteUrl={DEL_URL.librarycategory.api}
  ...
  getCategoryTypeList = () => {
    ...
    let url = GET_URL.librarycategory.api;
    let params = { is_active: 1, show_all_category: 1 };
            ...
            <Grid container>
              <Grid item md={7} xs={12} sm={12}>
                <div className="header-align heading">
                  <FormattedMessage {...messages.libCategoryType} />
                </div>
              ...
              <Grid item md={5} xs={12} sm={12}>
                <div className="end-flex-prop header-align">
                  {isUserHasPermission("library_categegory", "create") && (
                    <Button
                      variant="contained"
                      component={Link}
                      to={Actions.library_categegory.create.url}
                      className="editbutton-view"
                    >
                      <AddCircleOutlineOutlinedIcon className="visibility-icon" />{" "}
                      {Actions.library_categegory.create.label}
                    </Button>
                  )}
                </div>
              </Grid>
...
export default withRouter(LibCategoryList);
