...
import { Alert, getUrlParam, isUserHasPermission } from 'Includes/functions';
...
class TimetableRow extends Component {
    ...
    createTimetableSettings = async (academicYearId, timetableId, standardSectionId, rows) => {
        ...
        let staffSubjectMappingResp = await getRequest(GET_URL.staffSubMapping.api, staffSubMappingParams);
                    ...
                    for (let j = 0; j < daysList.length; j++) {
                        ...
                        timetableResp.data.data.forEach((data) => {
                            if (data.day == daysList[j].id && data.period == existingPeriodIds[i]) {
                                ...
                                tableSelects[key]['enable_edit'] = true;
                                tableSelects[key]['enable_delete'] = true;
                            }
                        });
                        lastIndexSelects++;
                    }
    ...
    componentDidMount = async () => {
        ...
        this.setState({
            ...
            rows: timetableSettings.rows, loading: !this.state.loading
        });
    }
        ...
        if (staffSubMappings && staffSubMappings.hasOwnProperty("subjectList")) {
            for (let j = 0; j < rows.length; j++) {
                ...
                for (let i = 0; i < rows[j]['rowSelects'].length; i++) {
                    ...
                    let cell = (<TableCell style={{ 'borderRight': '1px solid #E9E9EA', 'borderBottom': '1px solid #E9E9EA', padding: "0px" }}
                        ...
                        {(tableSelects && tableSelects[selectBoxKey] && tableSelects[selectBoxKey].enable_edit) ? (
                            <Box display="flex" mb={1} justifyContent="flex-end">
                                <Tooltip title="edit" aria-label="text">
                                    <EditIcon className="icon-color edit-icon"
                        ...
                        {(tableSelects && tableSelects[selectBoxKey] && tableSelects[selectBoxKey].enable_edit) ?
                            (<Tooltip title={staffSubMappings.subjectList[tableSelects[selectBoxKey].subject_id]} aria-label="text">
                                <Box className="table-cell-timetable">
                        ...
                        {(tableSelects && tableSelects[selectBoxKey] && tableSelects[selectBoxKey].enable_edit) ?
                            (<React.Fragment>
                                <Tooltip title={staffSubMappings.staffList[tableSelects[selectBoxKey].staff_id]} aria-label="text"><Typography variant="subtitle2" 
                                    gutterBottom style={{ "padding": "5px", "background": "#18A453", "color": "white", "border-radius": "3px", 
                                    'max-width': '9rem', 'width': '9rem', 'text-overflow': 'ellipsis', 'margin-left': 'auto', 'margin-right': 'auto',
                                    'margin-top': '12px', 'overflow': 'hidden', 'white-space': 'nowrap', 'margin-bottom': '12px', 'marginLeft': '5px', 'marginRight': '5px'}}>
                    ...
                    {loading ? (
                        <CircularProgress />
                            ...
                            <Box>
                                ...
                                <Box className="md-up-justify-space-between" pb={1}>
                                ...
                                <TableContainer component={Paper}>
                                    <Table className={classes.table} aria-label="timetable">
                                        ...
                                        <TableBody>
                                            {
                                                Cells && Cells.length ?
                                                    Cells : <TableCell colSpan={daysList.length}>No Periods Available, Click </TableCell>
                                            }
