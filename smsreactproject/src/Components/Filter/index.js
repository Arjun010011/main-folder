...
import {
    IconButton, Grid, Tooltip, Box, Menu, MenuItem,
    Button, TextField, Dialog, DialogActions, DialogContent,
    DialogContentText, DialogTitle, CircularProgress, FormControlLabel, Switch,
    TextareaAutosize, FormControl, FormHelperText
...
class Filter extends Component {
    ...
    handleClickOpen = () => {
        const { fieldDetails, fieldValues, isPricePlan, newEditData } = this.props;
        ...
        if (!isObjectEmpty(newEditData) && newEditData.redirectToUrl) {
            ...
            fieldDetails.map((fields, index) => {
                if (!fields.hide) {
                    data[fields.name] = fieldValues ? fieldValues[index] : '';
    ...
    handleClickView = () => {
        ...
        if (!isObjectEmpty(newViewData) && newViewData.redirectToUrl) {
            ...
            if (newViewData.params) {
                let searchParam = "?" + new URLSearchParams(newViewData.params).toString()
                pushData['search'] = searchParam
            }
            this.props.history.push(pushData);
        } else {
    ...
    editPricePlan = () => {
        let { yearName, year, planname, selectedStandards, isEdit, planid } = this.props
        this.props.history.push({
            pathname: Actions.transport_priceplan.create.url,
            state: {
                detail: {
                    yearName: yearName, year: year, planname: planname, selectedStandards: selectedStandards, isEdit: isEdit, planid: planid
                }
            }
        })
    }
    ...
    render() {
        const { open, showData, updateDisable,  fieldValue, errorContent, loadingData, options } = this.state
        const { enabledActions, baseClassName, label, fieldDetails, rowData, selected } = this.props
        const {
            handleClick, handleClose, handleCloseMenu, handleClickOpen, handleDeleteAndClose, handleSearchChange, onBlurValidation,
            update, handleApprove, handleReject, handleDisable
        } = this;
                        ...
                        <DialogContent className={loadingData ? 'display-none' : ''}>
                            ...
                            <Grid container className='flex-justify-center'>
                                {fieldDetails && fieldDetails.map((field) => <Grid item md={field.md} xs={10} sm={10}>
                                    ...
                                    {field.type === 'dropDown' && !loadingData && fieldValue[field.name] !== undefined &&
                                        <Dropdown
                                            data={field.list}
                                            name={field.name}
                                            value={fieldValue[field.name]}
                                    ...
                                    {field.type === 'dropDownWithSearch' && fieldValue[field.name] !== undefined && !loadingData &&
                                        <DropDownWithSearch
                                            id={`${field.name}`}
                                            options={field.list}
                                            value={fieldValue[field.name]}
                                            ...
                                            required={field.required}
                                            error={fieldValue[field.name + '_error']}
                                            disabled={field.parent ? fieldValue[field.parent] ? false : true : field.disabled}
                                            helperText={fieldValue[field.name + '_error'] ? fieldValue[field.name + '_error'] : field.helperText}
                                            hideClearIcon={field.hideClearIcon}
                                        />
                                    }
                                    {field.type === "checkbox" && fieldValue[field.name] !== undefined && (
                                        <Box>
                                            <Checkbox
                                                onChange={(e) =>
                                    ...
                                    {field.type === 'switch' &&
                                        <FormControlLabel
                                            control={<Switch checked={fieldValue[field.name]}
                                                name={field.name}
                                                value={fieldValue[field.name]}
                                                color="primary"
                                                onChange={(e) =>
