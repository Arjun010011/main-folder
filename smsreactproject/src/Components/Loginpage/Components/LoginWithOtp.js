...
export default class LoginWithOtp extends Component {
        ...
        if (response && response.status === 200) {
          ...
          if (response.data.data["userdetail"].length === 1) {
            ...
            this.setState(
              ...
              () => {
                this.handleLogin(fieldValue["username"]);
              }
            );
          } else {
            user_list = response.data.data["userdetail"];
  ...
  handleLogin = async (name) => {
    let { user_list } = this.state;
    let index = user_list.findIndex((data) => data["user"].username === name);
    let login_details = user_list[index];
    localStorage.setItem("token", login_details.token);
    localStorage.setItem("user", JSON.stringify(login_details.user));
    localStorage.setItem("menu", JSON.stringify(login_details.menu));
    localStorage.setItem("settings", JSON.stringify(login_details.settings));
  ...
  selectUser = async () => {
    ...
    this.handleLogin(selected_user);
  };
      ...
      <Box className="end-flex-prop">
        ...
        >
          ...
          <DialogContent>
            ...
            {errorContent && (
              <Box className="text-red fs-18 flex-justify-center margin-top-10">
                {errorContent}
              </Box>
            )}
